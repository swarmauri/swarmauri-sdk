...
import pytest
from swarmauri.community.tools.concrete.GithubWebhookTool import (
    GithubWebhookTool as Tool,
)
...
def test_ubc_type():
    token = os.getenv("GITHUBTOOL_TEST_TOKEN")
    assert Tool(token=token).type == "GithubWebhookTool"
...
@pytest.mark.unit
@patch("swarmauri.community.tools.concrete.GithubWebhookTool.Github")
