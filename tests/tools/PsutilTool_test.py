...
@pytest.mark.parametrize(
    ...
    [
        ('cpu', ['cpu_times', 'cpu_percent', 'cpu_times_per_cpu', 'cpu_count', 'cpu_frequency', 'cpu_stats'], False),
        ('memory', ['virtual_memory', 'swap_memory'], False),
        ('disk', ['disk_partitions', 'disk_usage', 'disk_io_counters'], False),
        ('network', ['network_io_counters', 'network_connections', 'network_interfaces'], False),
...
def test_call(info_type, expected_keys, should_raise):
    ...
    if should_raise:
        ...
        assert isinstance(result, dict), f"Expected result for {info_type} to be a dictionary"
