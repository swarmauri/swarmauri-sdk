...
@pytest.mark.parametrize(
    ...
    [
        ...
        6.1),  # Test case 3: more complex text
        ("", 0.0),  # Test case 4: empty string
        ("Text with only short words and sentences.", 1.8)  # Test case 5: very simple text
    ]
)
def test_call(input_text, expected_smog_index):
    ...
    assert isinstance(result.get("smog_index"),
                      float), f"Expected float, but got {type(result.get('smog_index')).__name__}"

    assert result.get("smog_index") == pytest.approx(expected_smog_index_calculated,
                                                     rel=0.1), f"Expected SMOG Index {pytest.approx(expected_smog_index_calculated, rel=0.1)}, but got {result.get('smog_index')}"