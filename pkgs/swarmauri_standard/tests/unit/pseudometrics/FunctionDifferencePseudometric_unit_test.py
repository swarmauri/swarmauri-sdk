...
def simple_functions() -> Dict[str, Callable]:
    ...
    return {
        ...
        "f2": lambda x: 2*x**2 if isinstance(x, (int, float)) else 2*sum(x.values()),
        "f3": lambda x: x**3 if isinstance(x, (int, float)) else sum(v**2 for v in x.values()),
...
def test_initialization_grid_points():
    ...
    # Check that sample points were generated
    assert pseudometric._sample_points is not None
    assert len(pseudometric._sample_points) <= 16  # Could be less if we have fewer grid points
