...
class TestSobolevMetric:
    ...
    @pytest.mark.parametrize(
        "order,weight,expected_order,expected_weight",
        [(1, 1.0, 1, 1.0), (2, 0.5, 2, 0.5), (0, 1.0, 0, 1.0)],
    )
    ...
    @pytest.mark.parametrize("order,weight", [(-1, 1.0), (1, -1.0), (0, 0.0)])
    def test_invalid_initialization(self, order, weight):
        ...
        logger = logging.getLogger(__name__)
        logger.debug(
            f"Testing invalid initialization with order={order}, weight={weight}"
        )
