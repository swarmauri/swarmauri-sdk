...
class TestValidation:
    ...
    def test_negative_values(self, hellingeraffinitysimilarity):
        ...
        with pytest.raises(ValueError, match="All values in probability vector must be non-negative"):
...
class TestSimilarity:
    ...
    @pytest.mark.parametrize("x, y, expected", [
        ...
        (np.array([0.5, 0.5]), np.array([0.0, 1.0]), 0.7071067811865475),  # Partially different
        (np.array([0.3, 0.7]), np.array([0.7, 0.3]), 0.86),  # Partially different (approx value)
    ])
...
class TestDissimilarity:
    ...
    @pytest.mark.parametrize("x, y, expected", [
        ...
        (np.array([0.5, 0.5]), np.array([0.0, 1.0]), 0.2928932188134525),  # Partially different
    ])
    def test_dissimilarity_calculation(self, hellingeraffinitysimilarity, x, y, expected):
