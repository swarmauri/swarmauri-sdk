...
class TestLInfNorm:
    ...
    @pytest.mark.parametrize("x,expected_norm", [
        ([1, 2, 3], 3),
        ([-1, -2, -3], 3),
        ([0, 0, 0], 0),
        (lambda x: x**2, 1),
    ...
    def test_check_absolute_homogeneity(self, norm_instance):
        ...
        assert np.isclose(norm_scaled, abs(alpha) * norm_x), "Absolute homogeneity failed"
    ...
    def test_serialization(self, norm_instance):
        ...
        assert norm_instance.id == loaded_json.id, "Serialization/Deserialization failed"