...
from swarmauri_standard.logger_handlers.EmailLoggingHandler import EmailLoggingHandler, CustomSMTPHandler
...
def test_compile_handler_basic(basic_email_handler):
    ...
    """
    with patch('swarmauri_standard.logger_handlers.EmailLoggingHandler.CustomSMTPHandler') as mock_handler:
...
def test_custom_smtp_handler_emit_ssl():
    ...
    with patch('smtplib.SMTP_SSL') as mock_smtp_ssl:
        ...
        assert not hasattr(smtp_instance, 'starttls') or not smtp_instance.starttls.called
...
@pytest.mark.unit
def test_custom_smtp_handler_emit_error_handling():
    """
    Test error handling in the emit metho