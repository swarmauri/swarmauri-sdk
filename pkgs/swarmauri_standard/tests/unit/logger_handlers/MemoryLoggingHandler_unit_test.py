...
def test_compile_handler_no_target():
    ...
    handler = MemoryLoggingHandler()
    with pytest.raises(ValueError, match="MemoryLoggingHandler requires a target handler"):
...
def test_compile_handler_string_target():
    ...
    with pytest.raises(ValueError, match="String target handlers not implemented: string_target"):
        handler.compile_handler()
...
def test_compile_handler_invalid_target():
    ...
    with pytest.raises(TypeError, match="Target must be a string or HandlerBase instance"):
        handler.compile_handler()
...
def test_compile_handler_with_custom_formatter():
    ...
    mock_formatter.compile_formatter.return_value = logging.Formatter("[TEST] %(message)s")
...
@pytest.mark.unit
@pytest.mark.parametrize("capacity,flush_level", [
    (10, logging.DEBUG),
    (100, logging.INFO),
