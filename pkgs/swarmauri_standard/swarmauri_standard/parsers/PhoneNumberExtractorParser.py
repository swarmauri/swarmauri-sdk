...
@ComponentBase.register_type(ParserBase, "PhoneNumberExtractorParser")
class PhoneNumberExtractorParser(ParserBase):
    ...
    def parse(self, data: str | Any) -> List[Document]:
        """
        Parses the input data, looking for phone numbers employing a regular
        expression.
        Each phone number found is contained in a separate IDocument instance.

        Parameters:
        - data (Union[str, Any]): The input text to be parsed for phone
          numbers.

        Returns:
        - List[IDocument]: A list of IDocument instances, each containing a
          phone number.
        """
        # Define a regular expression for phone numbers.
        # This is a simple example and might not capture all phone number
        # formats accurately.
        ...
        documents = [
            Document(content=phone_number, metadata={})
