...
class FunctionDifferencePseudometric(PseudometricBase):
    ...
    def __init__(
        ...
        if sampling_strategy == "fixed" and evaluation_points is None:
            raise ValueError("evaluation_points must be provided when sampling_strategy is 'fixed'")
        
        if sampling_strategy in ["random", "grid"] and domain_bounds is None:
            raise ValueError("domain_bounds must be provided when sampling_strategy is 'random' or 'grid'")
        ...
        logger.debug(f"Initialized FunctionDifferencePseudometric with strategy={sampling_strategy}, "
    ...
    def _generate_sample_points(self) -> None:
        ...
        logger.debug(f"Generated {len(self._sample_points)} sample points using {self.sampling_strategy} strategy")
    ...
    def _generate_random_points(self) -> None:
        ...
        for _ in range(self.num_samples):
            if len(dimensions) == 1:
                ...
                point = {dim: random.uniform(lower, upper) 
    ...
    def _generate_grid_points(self) -> None:
        ...
        if len(dimensions) == 1:
            ...
            if len(self._sample_points) > self.num_samples:
                self._sample_points = random.sample(self._sample_points, self.num_samples)
            ...
            try:
                ...
                if isinstance(value, (int, float)):
                    ...
                    if hasattr(value, 'norm'):
                        ...
                        raise ValueError(f"Function output {value} cannot be converted to a float")
            except Exception as e:
                logger.error(f"Error evaluating function at point {point}: {e}")
                raise ValueError(f"Failed to evaluate function at point {point}: {e}") from e
        
        return results
    
    def _calculate_difference(self, values1: List[float], values2: List[float]) -> float:
        """
    ...
    def distances(self, xs: Sequence[Callable], ys: Sequence[Callable]) -> List[List[float]]:
        """
    ...
    def check_symmetry(self, x: Callable, y: Callable, tolerance: float = 1e-10) -> bool:
        """
        ...
        try:
            ...
            if not result:
                logger.warning(f"Symmetry check failed: d(x,y) = {dist_xy}, d(y,x) = {dist_yx}")
    ...
    def check_triangle_inequality(self, x: Callable, y: Callable, z: Callable, 
        ...
        try:
            ...
            if not result:
                logger.warning(f"Triangle inequality check failed: "
