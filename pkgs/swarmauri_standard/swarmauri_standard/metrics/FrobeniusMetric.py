...
class FrobeniusMetric(MetricBase, IMetric):
    ...
    def distance(self, x: Union[IVector, IMatrix, np.ndarray], y: Union[IVector, IMatrix, np.ndarray]) -> float:
        """
        ...
        if x.shape != y.shape:
            raise ValueError("Matrices must have the same shape for Frobenius distance calculation")
    ...
    def distances(self, xs: List[Union[IVector, IMatrix, np.ndarray]], ys: List[Union[IVector, IMatrix, np.ndarray]]) -> List[List[float]]:
        """
        ...
        """
        logger.debug(f"Calculating pairwise Frobenius distances between {len(xs)} and {len(ys)} matrices")
    ...
    def check_non_negativity(self, x: Union[IVector, IMatrix, np.ndarray], y: Union[IVector, IMatrix, np.ndarray]) -> None:
        """
    ...
    def check_identity(self, x: Union[IVector, IMatrix, np.ndarray], y: Union[IVector, IMatrix, np.ndarray]) -> None:
        """
    ...
    def check_symmetry(self, x: Union[IVector, IMatrix, np.ndarray], y: Union[IVector, IMatrix, np.ndarray]) -> None:
        """
        ...
        if not np.isclose(distance_xy, distance_yx):
            raise ValueError(f"Symmetry axiom violated: d(x,y)={distance_xy}, d(y,x)={distance_yx}")

    def check_triangle_inequality(self, x: Union[IVector, IMatrix, np.ndarray], y: Union[IVector, IMatrix, np.ndarray], z: Union[IVector, IMatrix, np.ndarray]) -> None:
        """
        ...
        if distance_xz > distance_xy + distance_yz:
            raise ValueError(f"Triangle inequality violated: d(x,z)={distance_xz}, d(x,y)+d(y,z)={distance_xy + distance_yz}")
