...
class MaxSizeConversation(IMaxSize, ConversationBase):
    ...
    def add_message(self, message: IMessage):
        """Adds a message and ensures the conversation does not exceed the max
        size."""
    ...
    def _enforce_max_size_limit(self):
        ...
        If the current history size exceeds the maximum size, the oldest
        messages are removed.
        We pop two messages (one for the user's prompt, one for the assistant's
        response)
        """
