from typing import TypeVar, Union, Callable, Sequence, Literal, Any, Dict, Tuple
import logging
import numpy as np
...
class PointEvaluationSeminorm(SeminormBase):
    ...
    def __init__(self, point: Any, absolute: bool = True):
        """
        ...
        logger.debug(f"Initialized PointEvaluationSeminorm with point={point}, absolute={absolute}")
    ...
    def compute(self, x: InputType) -> float:
        ...
        try:
            ...
            if callable(x):
                ...
                raise TypeError(f"Unsupported input type: {type(x)}. Must be callable or support item access.")
            ...
            try:
                ...
                if hasattr(result, 'norm'):
                    ...
                    raise TypeError(f"Cannot convert result {result} to a non-negative real number")
            ...
            logger.error(f"Point {self.point} is not in the domain of the function: {e}")
            raise ValueError(f"Point {self.point} is not in the domain of the function") from e
    ...
    def check_scalar_homogeneity(self, x: InputType, alpha: T) -> bool:
        ...
        try:
            ...
            if callable(x):
                ...
                raise TypeError(f"Cannot scale input of type {type(x)} with scalar {alpha}")
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "PointEvaluationSeminorm":
        """
