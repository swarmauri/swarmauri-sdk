from typing import TypeVar, Union, Callable, Sequence, Literal, Set
import logging
import numpy as np
...
class CoordinateProjectionSeminorm(SeminormBase):
    ...
    def __init__(self, projection_indices: Set[int]):
        ...
        logger.info(f"Initialized CoordinateProjectionSeminorm with projection indices: {projection_indices}")
    ...
    def _compute_vector(self, x: IVector) -> float:
        ...
        if max_index >= len(components):
            raise ValueError(f"Projection index {max_index} out of bounds for vector of length {len(components)}")
        
        # Project onto the selected coordinates
        projected_components = [components[i] for i in self.projection_indices if i < len(components)]
    ...
    def _compute_matrix(self, x: IMatrix) -> float:
        ...
        if max_index >= len(flattened):
            raise ValueError(f"Projection index {max_index} out of bounds for flattened matrix of length {len(flattened)}")
        
        # Project onto the selected coordinates
        projected_components = [flattened[i] for i in self.projection_indices if i < len(flattened)]
    ...
    def _compute_sequence(self, x: Sequence) -> float:
        ...
        max_index = max(self.projection_indices) if self.projection_indices else -1
        if max_index >= len(x):
            raise ValueError(f"Projection index {max_index} out of bounds for sequence of length {len(x)}")
    ...
    def check_triangle_inequality(self, x: InputType, y: InputType) -> bool:
        ...
        if isinstance(x, IVector) and isinstance(y, IVector):
            ...
            raise TypeError(f"Unsupported or incompatible input types: {type(x)} and {type(y)}")
