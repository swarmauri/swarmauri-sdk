...
class HTTPSLoggingHandler(HandlerBase):
    ...
    def _create_ssl_context(self) -> ssl.SSLContext:
        ...
        context = ssl.create_default_context(
            purpose=ssl.Purpose.CLIENT_AUTH if self.cert_file else ssl.Purpose.SERVER_AUTH
        )
    ...
    def _format_record(self, record: logging.LogRecord) -> Dict[str, str]:
        ...
        for attr, value in record.__dict__.items():
            if attr not in log_data and not attr.startswith('_') and isinstance(value, (str, int, float, bool)):
