...
class HierarchicalAttentionModel(IModel):
    ...
    def load_model(self) -> None:
        ...
        word_attention_layer = tf.keras.layers.Attention(
            use_scale=True, return_attention_scores=True
        )
        word_attention_output, word_attention_weights = word_attention_layer(
        ...
        sentence_input = tf.keras.layers.Input(
            shape=(num_sentences, sentence_length), dtype="int32"
        )
        sentence_encoder_with_attention = tf.keras.layers.TimeDistributed(
            word_encoder_with_attention
        )(sentence_input)
        sentence_gru = tf.keras.layers.Bidirectional(
            tf.keras.layers.GRU(units, return_sequences=True)
        )(sentence_encoder_with_attention[0])
        sentence_attention_layer = tf.keras.layers.Attention(
            use_scale=True, return_attention_scores=True
        )
        sentence_attention_output, sentence_attention_weights = (
            sentence_attention_layer(
                [sentence_gru, sentence_gru], return_attention_scores=True
            )
        )
        doc_representation = tf.keras.layers.Dense(units, activation="tanh")(
            sentence_attention_output
        )
    ...
    def predict(self, input_data: Any) -> Any:
        ...
        if self._model is None:
            raise ValueError(
                "Model is not loaded. Call `load_model` before prediction."
            )
