...
class SpatialDocEmbedding(EmbeddingBase):
    ...
    def add_metadata(self, text, metadata_dict):
        ...
        for key, value in metadata_dict.items():
            if (
                f"[{key.upper()}]"
                in self._special_tokens_dict["additional_special_tokens"]
            ):
    ...
    def enhance_embedding_with_positional_info(
        self, embeddings, doc_position, total_docs
    ):
        position_effect = torch.sin(
            torch.tensor(doc_position / total_docs, dtype=torch.float)
        )
    ...
    def vectorize_document(self, chunks, metadata_list=None):
        ...
        for i, (chunk, metadata) in enumerate(zip(chunks, metadata_list)):
            ...
            enhanced_embeddings = self.enhance_embedding_with_positional_info(
                embeddings, i, total_chunks
            )
