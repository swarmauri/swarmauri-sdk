...
class ScannVectorStore(IVectorStore, ISimilarityQuery):
    """
    A vector store that utilizes ScaNN (Scalable Nearest Neighbors) for
    ...
    def __init__(
        ...
        - num_leaves (int): The number of leaves for the ScaNN partitioning
          tree.
        - num_leaves_to_search (int): The number of leaves to search for query
          time. Must be <= num_leaves.
        - reordering_num_neighbors (int): The number of neighbors to re-rank
          based on the exact distance after searching leaves.
        """
        ...
        # Placeholder for the ScaNN searcher initialized during building
        self.searcher = None
    ...
    def add_vector(
        ...
        """
        Adds a vector along with its identifier and optional metadata to the
        store.

        Args:
            vector_id (str): Unique identifier for the vector.
            vector (Union[np.ndarray, List[float]]): The high-dimensional
            vector to be stored.
        ...
        if self.searcher is None:
            ...
            raise Exception(
                (
                    "Cannot add vectors after building the index. Rebuild the "
                    "index to include new vectors."
                )
            )
    ...
    def build_index(self):
        """Builds or rebuilds the ScaNN searcher to reflect the current dataset
        vectors."""
    ...
    def get_vector(self, vector_id: str) -> Union[IVector, None]:
        ...
        Returns:
            Union[IVector, None]: The vector associated with the given id, or
            None if not found.
        """
    ...
    def delete_vector(self, vector_id: str) -> None:
        """
        Deletes a vector from the ScannVectorStore and marks the index for
        rebuilding.
        Note: For simplicity, this function assumes vectors are uniquely
        identifiable by their metadata.

        Args:
            vector_id (str): The unique identifier for the vector to be
            deleted.
        """
        if vector_id in self.id_to_metadata:
            ...
            # Since vector order is important for matching ids, rebuild the
            # searcher to reflect deletion
            self.searcher = None
        else:
    ...
    def update_vector(
        ...
        """
        Updates an existing vector in the ScannVectorStore and marks the index
        for rebuilding.
        ...
        Args:
            ...
            new_metadata (Dict, optional): Optional updated metadata for the
            vector.
        """
        ...
        if vector_id in self.id_to_metadata:
            # Update operation follows delete then add strategy because vector
            # order matters in ScaNN
    ...
    def search_by_similarity_threshold(
        ...
        """
        Search vectors exceeding a similarity threshold to a query vector
        within an optional vector space.

        Args:
            query_vector (Union[np.ndarray, List[float]]): The high-dimensional
            query vector.
            similarity_threshold (float): The similarity threshold for
            filtering results.
            space_name (str, optional): The name of the vector space to search
            within. Not used in this implementation.

        Returns:
            List[Dict]: A list of dictionaries with vector IDs, similarity
            scores, and optional metadata that meet the similarity threshold.
        """
