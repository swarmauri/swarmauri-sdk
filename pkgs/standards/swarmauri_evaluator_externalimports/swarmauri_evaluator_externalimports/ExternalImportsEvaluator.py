...
class ExternalImportsEvaluator(EvaluatorBase):
    ...
    def _initialize_standard_modules(self) -> None:
        ...
        self.standard_modules = {module.name for module in pkgutil.iter_modules(sys.stdlib_module_names)}
        ...
        additional_std_libs = {
            ...
            'copy', 'csv', 'datetime', 'decimal', 'email', 'enum', 'functools', 'gzip',
            'hashlib', 'http', 'importlib', 'inspect', 'io', 'itertools', 'json', 'logging',
            'math', 'multiprocessing', 'os', 'pathlib', 'pickle', 're', 'shutil', 'signal',
        ...
        logger.debug(f"Initialized with {len(self.standard_modules)} standard library modules")
    ...
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        for file_path in python_files:
            ...
            for imp in file_imports:
                ...
                if 'is_standard' in imp and not imp['is_standard'] and 'error' not in imp:
        ...
        for imp in external_imports:
            if imp['type'] == 'import':
                unique_external_modules.add(imp['module'])
            elif imp['type'] == 'importfrom':
                unique_external_modules.add(imp['parent_module'])
        ...
        logger.info(f"Found {len(external_imports)} external imports from {len(unique_external_modules)} unique modules")
