...
from swarmauri_evaluator_externalimports.ExternalImportsEvaluator import ExternalImportsEvaluator
...
""")
    ...
    try:
        ...
        non_standard_imports = [imp for imp in imports if not imp.get('is_standard', True)]
...
def test_initialize_standard_modules():
    ...
    common_std_modules = [
        ...
        "re", "math", "logging", "pathlib", "typing"
    ]
    
    for module in common_std_modules:
        assert module in evaluator.standard_modules, f"{module} should be in standard modules"
