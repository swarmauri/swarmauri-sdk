...
class AnyTypeUsageEvaluator(EvaluatorBase, ComponentBase):
    ...
    penalty_per_occurrence: float = Field(default=0.1, description="Penalty score per Any occurrence")
    max_penalty: float = Field(default=1.0, description="Maximum penalty that can be applied")
    ...
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        logger.info(f"Found {total_occurrences} Any type usages across {len(any_occurrences)} files")
    ...
    def _get_python_files(self, program: Program) -> List[str]:
        ...
        if hasattr(program, 'path') and program.path:
            ...
            root_dir = os.getcwd()
            logger.warning(f"Program path not available, using current directory: {root_dir}")
...
class AnyTypeVisitor(ast.NodeVisitor):
            ...
            for name in node.names:
                if name.name == 'Any':
                    self.any_occurrences.append((
                        node.lineno,
                        f"from typing import {name.name}"
                    ))
    ...
    def visit_Name(self, node):
        ...
        if node.id == 'Any' and not isinstance(node.ctx, ast.Store):
            ...
            context = f"Any used as identifier"
    ...
    def visit_arg(self, node):
        ...
        if node.annotation and isinstance(node.annotation, ast.Name) and node.annotation.id == 'Any':
    ...
    def visit_Subscript(self, node):
        ...
        if isinstance(node.value, ast.Name) and isinstance(node.slice, ast.Name) and node.slice.id == 'Any':
