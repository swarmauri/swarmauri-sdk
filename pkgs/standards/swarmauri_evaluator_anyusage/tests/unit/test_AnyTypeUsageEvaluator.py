...
@pytest.mark.unit
@pytest.mark.parametrize("num_occurrences,expected_score", [
...
def test_compute_score(evaluator, mock_program, num_occurrences, expected_score):
    ...
    with patch.object(evaluator, '_get_python_files') as mock_get_files, \
        ...
        if num_occurrences == 0:
            ...
            mock_analyze.side_effect = [
                [{"line": i, "context": f"context {i}"} for i in range(1, occurrences_per_file + 1)],
                [{"line": i, "context": f"context {i}"} for i in range(1, num_occurrences - occurrences_per_file + 1)]
            ]
...
def test_component_registration():
    ...
    # Check if AnyTypeUsageEvaluator is registered with the correct type
    registered_types = ComponentBase.get_registered_types(EvaluatorBase)
    assert "AnyTypeUsageEvaluator" in registered_types
    assert registered_types["AnyTypeUsageEvaluator"] == AnyTypeUsageEvaluator