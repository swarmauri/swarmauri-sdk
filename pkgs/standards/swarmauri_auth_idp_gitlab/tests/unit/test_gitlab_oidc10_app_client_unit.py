...
from swarmauri_auth_idp_gitlab import GitLabOIDC10AppClient
...
class StubClient:
    ...
    async def get_retry(self, url: str, **_: Any) -> StubResponse:
        self.calls += 1
        return StubResponse(self._payload)


@pytest.fixture
def client() -> GitLabOIDC10AppClient:
    instance = GitLabOIDC10AppClient(
...
@pytest.mark.unit
def test_resource(client: GitLabOIDC10AppClient) -> None:
    assert client.resource == "OIDC10AppClient"


@pytest.mark.unit
def test_type(client: GitLabOIDC10AppClient) -> None:
    assert client.type == "GitLabOIDC10AppClient"


@pytest.mark.unit
@pytest.mark.asyncio
async def test_access_token_cached(client: GitLabOIDC10AppClient) -> None:
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_access_token_refreshes(client: GitLabOIDC10AppClient) -> None:
