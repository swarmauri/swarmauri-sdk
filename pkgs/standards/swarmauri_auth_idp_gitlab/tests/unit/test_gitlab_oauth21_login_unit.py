...
from swarmauri_auth_idp_gitlab import GitLabOAuth21Login


@pytest.fixture
def login() -> GitLabOAuth21Login:
    instance = GitLabOAuth21Login(
...
@pytest.mark.unit
def test_resource(login: GitLabOAuth21Login) -> None:
    assert login.resource == "OAuth21Login"


@pytest.mark.unit
def test_type(login: GitLabOAuth21Login) -> None:
    assert login.type == "GitLabOAuth21Login"


@pytest.mark.unit
def test_serialization(login: GitLabOAuth21Login) -> None:
    serialized = login.model_dump_json()
    payload = json.loads(serialized)

    restored = GitLabOAuth21Login.model_construct(**payload)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_auth_url_uses_discovery(login: GitLabOAuth21Login) -> None:
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_exchange_invalid_state(login: GitLabOAuth21Login) -> None:
