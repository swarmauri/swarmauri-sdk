...
from swarmauri_auth_idp_google import GoogleOAuth21Login


@pytest.fixture
def login() -> GoogleOAuth21Login:
    instance = GoogleOAuth21Login(
        client_id="client",
...
@pytest.mark.unit
def test_resource(login: GoogleOAuth21Login) -> None:
    assert login.resource == "OAuth21Login"


@pytest.mark.unit
def test_type(login: GoogleOAuth21Login) -> None:
    assert login.type == "GoogleOAuth21Login"


@pytest.mark.unit
def test_serialization(login: GoogleOAuth21Login) -> None:
    serialized = login.model_dump_json()
    payload = json.loads(serialized)

    restored = GoogleOAuth21Login.model_construct(**payload)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_auth_url_uses_discovery(login: GoogleOAuth21Login) -> None:
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_exchange_invalid_state(login: GoogleOAuth21Login) -> None:
