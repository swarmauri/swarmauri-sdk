...
async def _invoke_runtime_dependency(request: Any, dep: Callable[..., Any]) -> Any:
    ...
    for name, param in inspect.signature(dep).parameters.items():
        ...
        if is_request_annotation(base_annotation) or name == "request":
            ...
            kwargs[name] = await _invoke_runtime_dependency(request, child)
            ...
            value, found = extract_param_value(marker, request, name, None)
            if found:
                kwargs[name] = value
    ...
    if inspect.isgenerator(out):
        try:
            ...
            return None
        state = getattr(request, "state", None)
        cleanups = getattr(state, "_dependency_cleanups", None)
...
def _make_runtime_dep_store(name: str, dep: Callable[..., Any]) -> Callable[..., Any]:
    async def _dep_store(request: Any) -> None:
        value = await _invoke_runtime_dependency(request, dep)
...
def _planned_route_deps(route: Any, handler: Callable[..., Any]) -> tuple[Any, ...]:
    ...
    for name, param in inspect.signature(handler).parameters.items():
        _, extras = split_annotated(param.annotation)
