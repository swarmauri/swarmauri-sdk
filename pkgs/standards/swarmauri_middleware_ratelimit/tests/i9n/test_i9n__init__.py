...
def test_init_module_loads():
    ...
    assert mod.__all__ == ["RateLimitMiddleware"], "__all__ should contain RateLimitMiddleware"
...
def test_rate_limit_middleware_import():
    ...
    try:
        ...
        assert isinstance(RateLimitMiddleware, type), "RateLimitMiddleware should be a class"
    except ImportError:
...
def test_init_module_contents():
    ...
    mod = import_module("swarmauri_middleware_ratelimit")
    assert hasattr(mod, "RateLimitMiddleware"), "RateLimitMiddleware should be in __init__"
    assert "RateLimitMiddleware" in mod.__all__, "RateLimitMiddleware should be in __all__"