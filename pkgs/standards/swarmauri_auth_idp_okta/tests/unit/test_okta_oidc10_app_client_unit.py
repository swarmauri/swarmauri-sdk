...
from swarmauri_auth_idp_okta import OktaOIDC10AppClient
...
@pytest.fixture
def client() -> OktaOIDC10AppClient:
    return OktaOIDC10AppClient(
...
@pytest.mark.unit
def test_resource(client: OktaOIDC10AppClient) -> None:
    assert client.resource == "OIDC10AppClient"


@pytest.mark.unit
def test_type(client: OktaOIDC10AppClient) -> None:
    assert client.type == "OktaOIDC10AppClient"


@pytest.mark.unit
def test_serialization(client: OktaOIDC10AppClient) -> None:
    dumped = json.loads(client.model_dump_json())
    cloned = OktaOIDC10AppClient.model_construct(**dumped)
