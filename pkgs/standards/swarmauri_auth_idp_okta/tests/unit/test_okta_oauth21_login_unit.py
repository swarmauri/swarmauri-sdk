...
from swarmauri_auth_idp_okta import OktaOAuth21Login


@pytest.fixture
def login() -> OktaOAuth21Login:
    instance = OktaOAuth21Login(
...
@pytest.mark.unit
def test_resource(login: OktaOAuth21Login) -> None:
    assert login.resource == "OAuth21Login"


@pytest.mark.unit
def test_type(login: OktaOAuth21Login) -> None:
    assert login.type == "OktaOAuth21Login"


@pytest.mark.unit
def test_serialization(login: OktaOAuth21Login) -> None:
    serialized = login.model_dump_json()
    payload = json.loads(serialized)

    restored = OktaOAuth21Login.model_construct(**payload)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_auth_url_uses_discovery(login: OktaOAuth21Login) -> None:
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_exchange_invalid_state(login: OktaOAuth21Login) -> None:
