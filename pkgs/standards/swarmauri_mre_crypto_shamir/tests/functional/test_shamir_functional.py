...
async def _run() -> bool:
    ...
    r1 = KeyRef(
        "r1", 1, KeyType.SYMMETRIC, (KeyUse.ENCRYPT,), ExportPolicy.PUBLIC_ONLY
    )
    r2 = KeyRef(
        "r2", 1, KeyType.SYMMETRIC, (KeyUse.ENCRYPT,), ExportPolicy.PUBLIC_ONLY
    )
    r3 = KeyRef(
        "r3", 1, KeyType.SYMMETRIC, (KeyUse.ENCRYPT,), ExportPolicy.PUBLIC_ONLY
    )
    env = await crypto.encrypt_for_many(
        [r1, r2, r3], b"functional", opts={"threshold_k": 2}
    )
    r4 = KeyRef(
        "r4", 1, KeyType.SYMMETRIC, (KeyUse.ENCRYPT,), ExportPolicy.PUBLIC_ONLY
    )
