...
from swarmauri_auth_idp_gitea import GiteaOAuth20Login


@pytest.fixture
def login() -> GiteaOAuth20Login:
    return GiteaOAuth20Login(
...
@pytest.mark.unit
def test_resource(login: GiteaOAuth20Login) -> None:
    assert login.resource == "OAuth20Login"


@pytest.mark.unit
def test_type(login: GiteaOAuth20Login) -> None:
    assert login.type == "GiteaOAuth20Login"


@pytest.mark.unit
def test_serialization(login: GiteaOAuth20Login) -> None:
    serialized = login.model_dump_json()
    data = json.loads(serialized)

    restored = GiteaOAuth20Login.model_construct(**data)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_exchange_invalid_state(login: GiteaOAuth20Login) -> None:
