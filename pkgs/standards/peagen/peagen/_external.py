...
def call_external_agent(
    ...
    Parameters:
      ...
      file_name (str, optional): Name of the file being generated. If the name
        ends with ``.md`` the result will not be chunked.
    ...
    content = chunk_content(result, logger, file_name)
    if logger:
...
def chunk_content(
    ...
    or the full content. If ``file_name`` ends with ``.md`` the text is
    returned without chunking.
    """
    ...
    if file_name and file_name.lower().endswith(".md"):
        if logger:
            logger.debug("Markdown file detected; skipping chunking")
        return cleaned_text
