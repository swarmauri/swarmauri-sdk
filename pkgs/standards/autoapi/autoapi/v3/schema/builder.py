...
def _build_schema(
    ...
    *,
    ...
    for col in table_cols:
        ...
        # Primary key columns are typically generated by the database and
        # should not be required in default "create" request schemas.  Allow
        # callers to explicitly include the PK via the ``include`` argument,
        # but otherwise skip it so POST payloads can omit the identifier.
        if (
            verb == "create"
            and getattr(col, "primary_key", False)
            and not (include and attr_name in include)
        ):
            continue
