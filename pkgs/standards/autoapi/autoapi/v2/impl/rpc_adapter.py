...
def _wrap_rpc(core, IN, OUT, pk_name: str, model):
    ...
    def _allowed_input_keys(IN) -> set[str]:
        ...
        for fi in fields.values():
            ...
            if isinstance(alias, str):
                allowed.add(alias)
            # validation_alias (AliasPath/AliasChoices) – best-effort extraction
    ...
    def h(raw: dict, db: Session):
        ...
        if expects_pydantic:
            # Avoid extra=forbid validation failures by prefiltering unknown keys
        ...
        # 2) Restore ANY server-injected mapped columns dropped by validation
        ...
        if expects_pydantic:
            ...
            if isinstance(raw_map, dict) and pk_name in raw_map and params and params[0].name != pk_name:
                ...
                # core(payload, db=...)  or  core(db=...) depending on signature
                if first_param and first_param.name not in (None, pk_name, "db"):
                    r = core(payload, db=db)
                else:
                    r = core(db=db)
        else:
            # Non-typed path (dicts) – preserve prior calling convention
