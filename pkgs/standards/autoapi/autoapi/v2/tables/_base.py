...
class Base(DeclarativeBase):
    def __init_subclass__(cls, **kw):
        ...
        if not hasattr(cls, "opspecs"):  cls.opspecs  = SimpleNamespace()
        if not hasattr(cls, "schemas"):  cls.schemas  = SimpleNamespace()
        if not hasattr(cls, "methods"):  cls.methods  = SimpleNamespace()
        if not hasattr(cls, "rpc"):      cls.rpc      = SimpleNamespace()
        if not hasattr(cls, "core"):     cls.core     = SimpleNamespace()
        if not hasattr(cls, "core_raw"): cls.core_raw = SimpleNamespace()
        if not hasattr(cls, "handlers"): cls.handlers = SimpleNamespace()
        if not hasattr(cls, "handlers_raw"): cls.handlers_raw = SimpleNamespace()
        if not hasattr(cls, "hooks"): cls.hooks = SimpleNamespace()
