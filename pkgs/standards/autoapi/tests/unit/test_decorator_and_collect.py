...
from autoapi.v3.engines.decorators import engine_ctx  # :contentReference[oaicite:17]{index=17}
from autoapi.v3.engines.autowire_collect import install_from_objects
from autoapi.v3.engines import resolver  # precedence registry  :contentReference[oaicite:18]{index=18}

# Minimal Table mixin expectation: the collector relies on presence of __tablename__
class TableMixin: __tablename__ = "t"
...
def test_install_and_resolve_op_vs_table():
    ...
    install_from_objects(models=[Model])  # registers table-level + op-level  :contentReference[oaicite:19]{index=19}
