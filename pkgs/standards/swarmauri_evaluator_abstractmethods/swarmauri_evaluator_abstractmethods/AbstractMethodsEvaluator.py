...
import logging
import os
from typing import Dict, Any, Tuple, List, Optional, Set, Literal
    ...
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        for file_path, source_code in source_files.items():
            ...
            if file_issues:
                ...
                total_abstract_classes += len({issue['class_name'] for issue in file_issues})
                total_methods += len(file_issues)
                total_missing_decorators += len([i for i in file_issues if not i['has_abstractmethod']])
                ...
                if isinstance(node, ast.ClassDef):
                    ...
                    if self._is_abstract_class(node):
                        ...
                        for method_node in [n for n in node.body if isinstance(n, ast.FunctionDef)]:
                            ...
                            if (self.ignore_dunder and method_name.startswith('__') and method_name.endswith('__')) or \
                               (self.ignore_private and method_name.startswith('_') and not method_name.startswith('__')):
                                continue
                            
                            # Check if the method has @abstractmethod decorator
                            has_abstractmethod = self._has_abstractmethod_decorator(method_node)
