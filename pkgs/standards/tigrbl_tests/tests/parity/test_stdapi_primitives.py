...
def test_response_helpers_and_status_line():
    payload = {"ok": True}
    response = Response.json(payload, status_code=201, headers={"X-Test": "1"})
    ...
    text_response = Response.text("ok")
    assert text_response.body == b"ok"


def test_http_exception_tracks_details():
    exc = HTTPException(status_code=403, detail="Forbidden", headers={"x": "y"})
