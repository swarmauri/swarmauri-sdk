...
async def test_secdeps_execute_before_txn_begin_and_handler(
    ...
    try:
        ...
        assert labels.count(f"hook:dep:security:0@{_ev.PRE_TX_SECDEP}") == 1
        assert labels.count(f"hook:dep:security:1@{_ev.PRE_TX_SECDEP}") == 1
        assert labels.count(f"hook:dep:extra:0@{_ev.PRE_TX_DEP}") == 1
