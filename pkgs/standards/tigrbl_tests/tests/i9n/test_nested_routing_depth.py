...
async def three_level_api_client(db_mode):
    ...
    class Department(Base, GUIDPk):
        ...
        def __tigrbl_nested_paths__(cls):
            return "/company/__/{company_id}/department"
    ...
    class Employee(Base, GUIDPk):
        ...
        @classmethod
        def __tigrbl_nested_paths__(cls):
            return "/company/__/{company_id}/department/__/{department_id}/employee"
...
async def test_nested_routing_depth(three_level_api_client):
    ...
    res = await client.post(
        f"/company/__/{company_id}/department",
    ...
    # Create employee
    res = await client.post(
        f"/company/__/{company_id}/department/__/{department_id}/employee",
        json={"name": "Alice"},
    )
    assert res.status_code == 201
    ...
    expected = {
        ...
        "/company/__/{item_id}": {"get", "patch", "delete"},
        "/company/__/{company_id}/department": {"post", "get", "delete"},
        "/company/__/{company_id}/department/__/{item_id}": {
            "get",
            "patch",
            "delete",
        },
        "/company/__/{company_id}/department/__/{department_id}/employee": {
            "post",
            "get",
            "delete",
        },
        "/company/__/{company_id}/department/__/{department_id}/employee/__/{item_id}": {
            "get",
            "patch",
            "delete",
        },
    ...
    res = await client.get(f"/company/__/{company_id}/department/__/{department_id}")
    assert res.status_code == 200
    assert res.json()["id"] == department_id

    res = await client.get(
        f"/company/__/{company_id}/department/__/{department_id}/employee/__/{employee_id}"
    )
    assert res.status_code == 200
