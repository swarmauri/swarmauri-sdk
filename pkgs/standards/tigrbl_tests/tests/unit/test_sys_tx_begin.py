...
@pytest.mark.asyncio
async def test_tx_begin_calls_installed_runner_and_sets_flag(
...
@pytest.mark.asyncio
async def test_tx_begin_noop_without_installed_runner(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_tx_begin_wraps_exceptions(monkeypatch: pytest.MonkeyPatch) -> None:
