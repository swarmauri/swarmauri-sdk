...
from tigrbl import TableBase, TigrblApp, TigrblRouter, alias_ctx
...
# Helper to bootstrap API and test client for a table
...
async def test_make_column_only_rest_rpc(use_mapped):
    ...
    try:
        ...
        resp = client.get(f"/api/{Thing.__name__.lower()}/{created['id']}")
