...
class TestMountFaviconOnTigrblApp:
    ...
    def test_mount_favicon_exposes_default_favicon_route(self) -> None:
    ...
    def test_mount_favicon_supports_custom_path(self) -> None:
...
class TestMountFaviconOnTigrblApi:
    ...
    def test_bound_mount_favicon_supports_api_instances(self) -> None:
        ...
        with TestClient(api) as client:
            response = client.get("/branding/favicon.ico")

        assert response.status_code == 200
        assert response.headers["content-type"].startswith("image/svg+xml")

    def test_bound_mount_favicon_retains_default_fallback(self) -> None:
        """Passing ``None`` for favicon path should use the default asset."""
        api = TigrblApi()
        api.mount_favicon(path="/fallback/favicon.ico", favicon_path=None)

        with TestClient(api) as client:
            response = client.get("/fallback/favicon.ico")
