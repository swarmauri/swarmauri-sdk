...
def opview_from_ctx(ctx: Any):
    ...
    model = getattr(ctx, 'model', None) or (type(getattr(ctx, 'obj', None)) if getattr(ctx, 'obj', None) is not None else None)
    ...
    if _kernel is not None and alias:
        ...
        if specs is not None:
            return _kernel._default_kernel._compile_opview_from_specs(specs, SimpleNamespace(alias=alias))
...
def ensure_schema_in(ctx: Any, ov) -> Mapping[str, Any]:
    ...
    if 'schema_in' not in temp:
        ...
        temp['schema_in'] = {'fields': ov.schema_in.fields, 'by_field': bf, 'required': req}
    return temp['schema_in']

def ensure_schema_out(ctx: Any, ov) -> Mapping[str, Any]:
    temp = _ensure_temp(ctx)
    if 'schema_out' not in temp:
        temp['schema_out'] = {'fields': ov.schema_out.fields, 'by_field': ov.schema_out.by_field, 'expose': ov.schema_out.expose}
