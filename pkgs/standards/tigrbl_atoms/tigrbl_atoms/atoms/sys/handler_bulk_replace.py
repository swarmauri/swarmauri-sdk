...
async def _run(obj: object | None, ctx: Any) -> None:
    ...
    if not isinstance(model, type):
        raise TypeError("handler_bulk_replace requires a model type")
    payload = _ctx.payload(ctx)
    if not isinstance(payload, list):
        raise TypeError("bulk_replace expects a list payload")
    setattr(ctx, "result", await _core.bulk_replace(model, payload, db=_ctx.db(ctx)))


class AtomImpl(Atom[Resolved, Operated]):
    name = "sys.handler_bulk_replace"
    anchor = ANCHOR
