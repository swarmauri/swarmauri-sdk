...
def _run(obj: object | None, ctx: Any) -> None:
    ...
    route_to_program = getattr(packed, 'route_to_program', None) if packed is not None else None

    program_id = -1
    if isinstance(proto_id, int) and isinstance(selector_id, int) and route_to_program is not None:
