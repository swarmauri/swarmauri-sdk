...
from swarmauri_tokens_tlsboundjwt import (
...
def test_verify_mismatch_functional():
    svc_a = TlsBoundJWTTokenService(
        None, client_cert_der_getter=lambda: b"cert-a"
    )
    token = asyncio.run(_mint(svc_a))
    svc_b = TlsBoundJWTTokenService(
        None, client_cert_der_getter=lambda: b"cert-b"
    )
...
def test_cnf_included_functional():
    svc = TlsBoundJWTTokenService(
        None, client_cert_der_getter=lambda: b"cert-x"
    )
    token = asyncio.run(_mint(svc))
