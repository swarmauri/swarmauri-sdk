...
from swarmauri_evaluatorpool_accessibility.ColemanLiauIndexEvaluator import ColemanLiauIndexEvaluator
...
def test_type():
    ...
    assert ColemanLiauIndexEvaluator.type == "ColemanLiauIndexEvaluator"
...
@pytest.mark.unit
@pytest.mark.parametrize("text,expected_letters", [
    ("Hello world", 10),
    ("123 numbers!", 7),
    ("", 0),
    ("   ", 0),
...
@pytest.mark.unit
@pytest.mark.parametrize("grade_level,expected_score", [
...
def test_calculate_score(evaluator, grade_level, expected_score):
    ...
    assert evaluator._calculate_score(grade_level) == pytest.approx(expected_score, abs=0.01)
...
@pytest.mark.unit
@pytest.mark.parametrize("program_attrs,expected_text", [
...
@pytest.mark.unit
@pytest.mark.parametrize("text,expected_grade_level", [
    ("Simple short sentences. Easy words.", 3),
    ("The Coleman-Liau index is a readability assessment that approximates the U.S. grade level thought necessary to comprehend the text.", 12),
    ("Hello world.", 5),
])
def test_compute_score_various_texts(evaluator, mock_program, text, expected_grade_level):
    ...
    assert abs(metadata["grade_level"] - expected_grade_level) <= 2  # Allow some flexibility in expected grade level
    
    # Check metadata contains expected keys
    expected_keys = {"grade_level", "raw_index", "letters", "words", "sentences", 
                    "letters_per_100_words", "sentences_per_100_words", "target_grade_level"}
