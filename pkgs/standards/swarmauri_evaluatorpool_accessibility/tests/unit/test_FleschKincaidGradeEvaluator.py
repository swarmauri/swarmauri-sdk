...
from swarmauri_evaluatorpool_accessibility.FleschKincaidGradeEvaluator import FleschKincaidGradeEvaluator
...
@pytest.mark.unit
@pytest.mark.parametrize("text, expected_syllables", [
    ("This is a test", 4),
    ("Hello world", 3),
    ("", 0),
...
@pytest.mark.unit
@pytest.mark.parametrize("output, expected_text", [
...
def test_formula_calculation(evaluator, mock_program):
    ...
    expected_score = max(0, 0.39 * words_per_sentence + 11.8 * syllables_per_word - 15.59)
    
    # Allow for small floating-point differences
    assert abs(score - expected_score) < 0.001
    
    # Verify formula components
    assert abs(metadata["formula_calculation"]["term1"] - (0.39 * words_per_sentence)) < 0.001
    assert abs(metadata["formula_calculation"]["term2"] - (11.8 * syllables_per_word)) < 0.001
