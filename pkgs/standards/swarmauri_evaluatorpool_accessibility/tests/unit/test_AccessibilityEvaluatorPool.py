...
def mock_evaluator():
    ...
    evaluator.evaluate_file.return_value = {"score": 0.75}
    evaluator.aggregate_results.return_value = {"score": 0.8}
    return evaluator
...
def mock_file_node():
    """
    Create a mock file node instance.
    
    Returns
    -------
    Mock
        A mock FileNode with test content
    """
    file_node = Mock(spec=FileNode)
    file_node.path = "test_file.py"
    file_node.content = "def test_function():\n    pass"
    return file_node


@pytest.fixture
def mock_program(mock_file_node):
    """
    ...
    ----------
    mock_file_node : Mock
        The mock file node to include in the program
    ...
    program.files = {"test_file.py": mock_file_node}
    return program
...
def test_evaluate(evaluator_pool, mock_program, mock_evaluator):
    ...
    mock_evaluator.evaluate_file.assert_called_once()


@pytest.mark.unit
def test_evaluate_file(evaluator_pool, mock_file_node, mock_evaluator):
    ...
    with patch.object(evaluator_pool, '_should_evaluate_file', return_value=True):
        score = evaluator_pool._evaluate_file(mock_file_node)
        
        assert score == 0.75
        mock_evaluator.evaluate_file.assert_called_once_with(mock_file_node)


@pytest.mark.unit
def test_evaluate_file_skipped(evaluator_pool, mock_file_node, mock_evaluator):
...
@pytest.mark.unit
def test_evaluate_file_exception(evaluator_pool, mock_file_node, mock_evaluator):
...
@pytest.mark.unit
@pytest.mark.parametrize("file_path, content, expected", [
    ("test.py", "content", True),
    ("test.bin", "content", False),
    ("test.jpg", "content", False),
    ("test.py", None, False),
    ("test.txt", "x" * 2_000_000, False),  # Exceeds size limit
])
def test_should_evaluate_file(file_path, content, expected):
    ...
    assert pool._should_evaluate_file(file_node) == expected
