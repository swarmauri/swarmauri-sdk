...
class FleschReadingEaseEvaluator(EvaluatorBase, ComponentBase):
    ...
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        logger.info(f"Flesch Reading Ease score: {fre_score:.2f} ({metadata['readability_interpretation']})")
    ...
    def _count_syllables(self, word: str) -> int:
        ...
        if self.cmu_dict and word in self.cmu_dict:
            ...
            return max(1, sum(1 for phoneme in self.cmu_dict[word][0] if any(c.isdigit() for c in phoneme)))
