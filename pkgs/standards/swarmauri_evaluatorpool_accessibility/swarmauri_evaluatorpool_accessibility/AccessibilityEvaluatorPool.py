import logging
from typing import Dict, List, Literal, Optional, Any, Union, cast
from pathlib import Path

from swarmauri_core.ComponentBase import ComponentBase
from swarmauri_core.EvaluatorPoolBase import EvaluatorPoolBase
from swarmauri_core.Program import Program
from swarmauri_core.EvaluatorBase import EvaluatorBase
from swarmauri_core.FileNode import FileNode

logger = logging.getLogger(__name__)


@ComponentBase.register_type("AccessibilityEvaluatorPool")
class AccessibilityEvaluatorPool(EvaluatorPoolBase):
    ...
    def __init__(
        ...
        **kwargs : Any
        ...
        logger.info(f"Initialized AccessibilityEvaluatorPool with {len(self.evaluators)} evaluators")
    ...
    def evaluate(self, program: Program) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting accessibility evaluation on program with {len(program.files)} files")
        ...
        for file_path, file_node in program.files.items():
            file_score = self._evaluate_file(file_node)
        ...
        logger.info(f"Completed accessibility evaluation with overall score: {overall_score:.2f}")
        return result
    
    def _evaluate_file(self, file_node: FileNode) -> float:
        """
        ...
        if not self._should_evaluate_file(file_node):
            logger.debug(f"Skipping file {file_path} as it's not suitable for accessibility evaluation")
            return 0.0
        ...
        for evaluator in self.evaluators:
            try:
                result = evaluator.evaluate_file(file_node)
                score = result.get("score", 0.0)
                file_scores.append(score)
                logger.debug(f"Evaluator {evaluator.__class__.__name__} gave score {score:.2f} for {file_path}")
            except Exception as e:
                logger.error(f"Error evaluating {file_path} with {evaluator.__class__.__name__}: {str(e)}")
                file_scores.append(0.0)
        
        # Calculate average score for this file
        if file_scores:
            avg_score = sum(file_scores) / len(file_scores)
            logger.debug(f"Average accessibility score for {file_path}: {avg_score:.2f}")
            return avg_score
        return 0.0
    
    def _should_evaluate_file(self, file_node: FileNode) -> bool:
        """
        ...
        if len(file_node.content) > 1_000_000:  # 1MB limit
            logger.warning(f"File {file_node.path} exceeds size limit for accessibility evaluation")
            return False
            
        # Skip file types that don't make sense for accessibility evaluation
        excluded_extensions = {'.bin', '.pyc', '.pyo', '.so', '.dll', '.exe', 
                              '.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico',
                              '.mp3', '.mp4', '.avi', '.mov', '.zip', '.tar', 
                              '.gz', '.rar'}
        
        file_ext = Path(file_node.path).suffix.lower()
