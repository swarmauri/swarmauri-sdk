...
class AutomatedReadabilityIndexEvaluator(EvaluatorBase, ComponentBase):
    ...
    """
    type: Literal["AutomatedReadabilityIndexEvaluator"] = "AutomatedReadabilityIndexEvaluator"
    ...
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        if not text_content:
            ...
            return 0.0, {"error": "No text content found", "chars": 0, "words": 0, "sentences": 0}
        ...
        logger.debug(f"Text statistics: {char_count} chars, {word_count} words, {sentence_count} sentences")
        
        # Handle edge cases to avoid division by zero
        if word_count == 0 or sentence_count == 0:
            logger.warning("Cannot compute ARI: division by zero (no words or sentences)")
            return 0.0, {"error": "Cannot compute ARI: no words or sentences", 
                         "chars": char_count, "words": word_count, "sentences": sentence_count}
