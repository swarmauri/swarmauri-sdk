...
class ColemanLiauIndexEvaluator(EvaluatorBase, ComponentBase):
    ...
    target_grade_level: int = Field(default=8, description="Target grade level for optimal readability")
    max_grade_level: int = Field(default=16, description="Maximum grade level to consider")
    ...
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        if not text or text.isspace():
            ...
            return 0.0, {"error": "Empty text", "grade_level": 0, "letters": 0, "words": 0, "sentences": 0}
        ...
        logger.debug(f"Text analysis: {letters} letters, {words} words, {sentences} sentences")
        
        # Calculate Coleman-Liau Index
        if words == 0:
            logger.warning("No words found in text")
            return 0.0, {"error": "No words found", "grade_level": 0, "letters": 0, "words": 0, "sentences": 0}
        ...
        logger.debug(f"Coleman-Liau Index: {coleman_liau_index:.2f}, Grade Level: {grade_level}")
