...
class MinkowskiDistance(DistanceBase):
    """
    Implementation of the IDistanceSimiliarity interface using the Minkowski
    distance metric.
    Minkowski distance is a generalized metric form that includes Euclidean
    distance,
    Manhattan distance, and others depending on the order (p) parameter.

    The class provides methods to compute the Minkowski distance between two
    vectors.

    Parameters:
    - p (int): The order of the Minkowski distance. p=2 corresponds to the
      Euclidean distance,
    ...
    def distance(self, vector_a: Vector, vector_b: Vector) -> float:
        ...
        Returns:
            float: The computed Minkowski distance between vector_a and
            vector_b.
        """
    ...
    def similarity(self, vector_a: Vector, vector_b: Vector) -> float:
        """
        Compute the similarity between two vectors based on the Minkowski
        distance.
