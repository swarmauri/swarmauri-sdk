# noqa: E501
"""Tests for OAuth 2.0 Bearer Token Usage compliance with RFC 6750.

The full RFC 6750 specification text is included below to ensure the tests
...
from tigrbl_auth.fastapi_deps import get_current_principal, get_db
...
async def test_lowercase_bearer_scheme():
    ...
    mock_user = MagicMock()
    with patch(
        "tigrbl_auth.fastapi_deps._user_from_jwt", AsyncMock(return_value=mock_user)
    ):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_access_token_query_parameter_enabled():
    ...
    with patch.object(settings, "enable_rfc6750_query", True):
        with patch(
            "tigrbl_auth.fastapi_deps._user_from_jwt",
