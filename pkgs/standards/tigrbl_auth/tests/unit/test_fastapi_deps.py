"""
Unit tests for tigrbl_auth.fastapi_deps module.
...
from tigrbl_auth.fastapi_deps import (
    ...
    async def test_user_from_jwt_with_valid_token(self):
        ...
        with (
            ...
            patch(
                ...
                new=AsyncMock(return_value=SimpleNamespace(items=[mock_user])),
            ) as mock_list,
        ):
...
class TestAPIKeyUserResolution:
    ...
    async def test_user_from_api_key_with_valid_key(self):
        ...
        with patch("tigrbl_auth.fastapi_deps._api_key_backend") as mock_backend:
...
class TestGetCurrentPrincipal:
    ...
    async def test_get_current_principal_with_valid_api_key(self):
        ...
        with patch(
            "tigrbl_auth.fastapi_deps._user_from_api_key", return_value=mock_user
        ):
    ...
    async def test_get_current_principal_with_valid_jwt(self):
        ...
        with patch("tigrbl_auth.fastapi_deps._user_from_jwt", return_value=mock_user):
            request = Request(scope={"type": "http"})
            principal = await get_current_principal(
                request,
    ...
    async def test_get_current_principal_api_key_takes_precedence(self):
        ...
        with patch(
            ...
            with patch(
                "tigrbl_auth.fastapi_deps._user_from_jwt", return_value=mock_user
            ) as mock_jwt:
                request = Request(scope={"type": "http"})
                principal = await get_current_principal(
                    request,
    ...
    async def test_get_current_principal_falls_back_to_jwt(self):
        ...
        with patch("tigrbl_auth.fastapi_deps._user_from_api_key", return_value=None):
            with patch(
                "tigrbl_auth.fastapi_deps._user_from_jwt", return_value=mock_user
            ):
                request = Request(scope={"type": "http"})
    ...
    async def test_get_current_principal_with_invalid_credentials(self):
        ...
        with patch("tigrbl_auth.fastapi_deps._user_from_api_key", return_value=None):
            with patch("tigrbl_auth.fastapi_deps._user_from_jwt", return_value=None):
                request = Request(scope={"type": "http"})
                with pytest.raises(HTTPException) as exc_info:
                    await get_current_principal(
                        request,
...
class TestGetPrincipal:
    ...
    async def test_get_principal_with_valid_credentials(self):
        ...
        with patch(
            "tigrbl_auth.fastapi_deps.get_current_principal", return_value=mock_user
        ):
    ...
    async def test_get_principal_with_authentication_failure(self):
        ...
        with patch(
            "tigrbl_auth.fastapi_deps.get_current_principal"
        ) as mock_get_current:
...
class TestFastAPIDepsIntegration:
    ...
    def test_backend_instances_are_created(self):
        ...
        from tigrbl_auth.fastapi_deps import _api_key_backend, _jwt_coder
