...
port. The tests below assert that tigrbl_auth enforces these rules.
...
def test_client_new_allows_public_redirect_when_disabled(monkeypatch) -> None:
    ...
    monkeypatch.setenv("TIGRBL_AUTH_ENFORCE_RFC8252", "0")
    importlib.reload(runtime_cfg)
    import tigrbl_auth.orm as orm_tables
    import tigrbl_auth.orm.client as orm_client
    ...
    monkeypatch.setenv("TIGRBL_AUTH_ENFORCE_RFC8252", "1")
    importlib.reload(runtime_cfg)
