...
def mock_program():
    ...
    program.get_path.return_value = "/path/to/program"
    program.is_executable.return_value = True
    return program
...
@pytest.mark.unit
def test_prepare_command_executable(mock_program):
...
@pytest.mark.unit
def test_prepare_command_python_script(mock_program):
...
@pytest.mark.unit
def test_prepare_command_shell_script(mock_program):
...
@pytest.mark.unit
def test_prepare_command_make_executable(mock_program):
...
@pytest.mark.unit
def test_calculate_score_exit_code_failure():
...
@pytest.mark.unit
def test_calculate_score_output_mismatch():
...
@pytest.mark.unit
def test_calculate_score_output_match():
...
def test_compute_score_success(mock_calculate, mock_execute, mock_prepare, mock_temp_dir, evaluator, mock_program):
    """Test successful score computation."""
    mock_temp_dir.return_value.name = "/tmp/test_dir"
    mock_prepare.return_value = ["cmd", "arg"]
    mock_execute.return_value = {"stdout": "out", "stderr": "err", "exit_code": 0, "timed_out": False}
    mock_calculate.return_value = (1.0, {"reason": "success"})
    
    score, metadata = evaluator._compute_score(mock_program, args=["arg"], input_data="input")
...
def test_compute_score_with_working_dir(evaluator, mock_program):
    ...
    with patch.object(SubprocessEvaluator, '_prepare_command') as mock_prepare, \
        ...
        mock_execute.return_value = {"stdout": "", "stderr": "", "exit_code": 0, "timed_out": False}
        mock_calculate.return_value = (1.0, {})
        
        evaluator._compute_score(mock_program)
        
        mock_execute.assert_called_once_with(ANY, input_data='', timeout=30.0, working_dir="/custom/dir")
...
def test_compute_score_custom_timeout(custom_timeout, evaluator, mock_program):
    ...
    with patch.object(SubprocessEvaluator, '_prepare_command') as mock_prepare, \
        ...
        expected_timeout = custom_timeout if custom_timeout is not None else evaluator.timeout
        mock_execute.assert_called_once_with(ANY, input_data='', timeout=expected_timeout, working_dir=ANY)
