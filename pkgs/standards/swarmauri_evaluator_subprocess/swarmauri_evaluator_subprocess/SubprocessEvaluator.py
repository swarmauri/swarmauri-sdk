...
class SubprocessEvaluator(EvaluatorBase):
    ...
    env_vars: Dict[str, str] = Field(default_factory=dict, description="Environment variables for the subprocess")
    
    # Scoring parameters
    success_exit_codes: List[int] = Field(default=[0], description="Exit codes considered successful")
    score_on_timeout: float = Field(default=0.0, description="Score to assign on timeout")
    score_on_error: float = Field(default=0.0, description="Score to assign on execution error")
    
    def _compute_score(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        """
        ...
        try:
            ...
            score, metadata = self._calculate_score(result, expected_output, execution_time)
    ...
    def _execute_subprocess(
        ...
        def limit_resources():
            ...
            if timeout:
                resource.setrlimit(resource.RLIMIT_CPU, (int(timeout * 0.9), int(timeout)))
    ...
    def aggregate_scores(self, scores: List[float], metadata_list: List[Dict[str, Any]]) -> Tuple[float, Dict[str, Any]]:
        """
        ...
        aggregated_score, aggregated_metadata = super().aggregate_scores(scores, metadata_list)
        ...
        if metadata_list:
            ...
            timeout_count = sum(1 for meta in metadata_list if meta.get("timed_out", False))
            timeout_rate = timeout_count / len(metadata_list) if metadata_list else 0
            
            # Calculate exit code statistics
            exit_codes = [meta.get("exit_code") for meta in metadata_list if "exit_code" in meta]
            success_exit_count = sum(1 for code in exit_codes if code in self.success_exit_codes)
