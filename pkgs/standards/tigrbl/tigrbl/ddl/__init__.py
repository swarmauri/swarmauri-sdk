...
def initialize(
    ...
    *,
    ...
    def _bootstrap(db):
        ...
        # Keep ``obj.tables`` keyed to model classes. Downstream routing,
        # diagnostics, and user-facing APIs rely on ``tables`` being model
        # registries (not SQLAlchemy ``Table`` objects).
