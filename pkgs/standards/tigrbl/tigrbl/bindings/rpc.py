...
def _build_rpc_callable(model: type, sp: OpSpec) -> Callable[..., Awaitable[Any]]:
    ...
    async def _rpc_method(
        ...
        *,
        ...
        # 2) Dispatch via shared transport executor path
        base_ctx: Dict[str, Any] = dict(ctx or {})
        app_ref = getattr(request, "app", None) or base_ctx.get("app") or model
        base_ctx.setdefault("api", base_ctx.get("api") or app_ref)

        model_hooks = getattr(getattr(model, "hooks", None), alias, None)
        post_response_steps = list(getattr(model_hooks, "POST_RESPONSE", []) or [])

        return await dispatch_operation(
            request=request,
            db=db,
            api=base_ctx.get("api"),
            model=model,
            alias=alias,
            target=target,
            ...
            post_response_steps=post_response_steps,
        )
