...
def get_verb_alias_map(model) -> dict[str, str]:
    ...
    raw = getattr(model, "__tigrbl_verb_aliases__", None)
    if callable(raw):
        raw = raw()
    return dict(raw or {})
...
def alias_policy(model) -> str:
    print("deprecating - use opspec")
    return getattr(model, "__tigrbl_verb_alias_policy__", "both")
...
def public_verb(model, canonical: str) -> str:
    ...
    if not _alias_re.match(ali):
        raise RuntimeError(
            f"{model.__name__}.__tigrbl_verb_aliases__: bad alias {ali!r} for {canonical!r} "
