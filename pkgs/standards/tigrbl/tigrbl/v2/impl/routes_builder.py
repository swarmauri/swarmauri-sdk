"""
tigrbl/v2/routes_builder.py  –  Route building functionality for Tigrbl.
...
def _register_routes_and_rpcs(  # noqa: N802 – bound as method
    ...
    allow_cb = getattr(model, "__tigrbl_allow_anon__", None)
                ...
                if db is None:  # auto-open sync session
                    if _api.get_db is None:
                        raise TypeError(
                            ...
                            "or register get_db when constructing Tigrbl()"
                        )
                    gen = _api.get_db()
                    db_ = next(gen)
                    try:
    ...
    # Keep a per-Tigrbl binding registry to avoid collisions when multiple APIs bind the same model
    _b = getattr(decl, "__tigrbl__", None)
    if _b is None:
        _b = SimpleNamespace()
        setattr(decl, "__tigrbl__", _b)
