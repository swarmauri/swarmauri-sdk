...
def collect_all_specs_for_table(table: Type) -> List[OpSpec]:
    ...
    if issubclass(table, OpConfigProvider):
        out.extend(list(getattr(table, "__tigrbl_ops__", ()) or []))
    ...
    for name in dir(table):
        try:
            ...
            continue
        tag: OpSpec | None = getattr(meth, "__tigrbl_custom_op__", None)
        if tag:
    ...
    # 4) back-compat: __tigrbl_verb_aliases__ → specs (RPC/method aliases only)
    raw = getattr(table, "__tigrbl_verb_aliases__", {}) or {}
    policy = getattr(table, "__tigrbl_verb_alias_policy__", "both")
    if callable(raw):
        raw = raw()
        ...
        if spec.expose_routes:
            ...
            async def _impl(**kw):
                ...
                if spec.persist == "skip":
                    req.state.ctx["__tigrbl_skip_persist__"] = True
            ...
            def _runner(payload, *, db=None, _method=method_id, _api=api):
                ...
                if db is None:
                    if _api.get_db is None:
                        raise TypeError(
                            "Supply a Session via db=... or configure get_db on Tigrbl()"
                        )
                    gen = _api.get_db()
                    db_ = next(gen)
                    try:
