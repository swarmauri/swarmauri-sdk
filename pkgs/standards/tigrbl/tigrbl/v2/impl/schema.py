"""
tigrbl/v2/impl/schema.py  –  Schema building functionality for Tigrbl.

Builds verb-specific Pydantic models from SQLAlchemy ORM classes, with:
• Column.info["tigrbl"] support (read_only, disable_on, no_update, …)
• Hybrid @hybrid_property opt-in via meta["hybrid"]
• Request-only virtual fields via __tigrbl_request_extras__ on the ORM class
"""
...
def _merge_request_extras(
    ...
    *,
    ...
    """
    buckets = getattr(orm_cls, "__tigrbl_request_extras__", None)
    if not buckets:
        return
...
def _merge_response_extras(
    ...
    *,
    ...
    buckets = getattr(orm_cls, "__tigrbl_response_extras__", None)
    if not buckets:
        return
...
def _schema(
    ...
    *,
    ...
    Keeps support for:
      • Column.info["tigrbl"] (read_only, write_only, disable_on, no_update, …)
      • @hybrid_property opt-in via meta["hybrid"]
      • __tigrbl_request_extras__ / __tigrbl_response_extras__
    """
    ...
    for col in table_cols:
        ...
        meta = (meta_src.get("tigrbl") if isinstance(meta_src, dict) else None) or {}
    ...
    for attr in (
        ...
        # Allow attaching meta to the hybrid itself (e.g., attr.info = {"tigrbl": {...}})
