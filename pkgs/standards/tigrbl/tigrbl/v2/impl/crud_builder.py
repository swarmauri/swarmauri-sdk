"""
tigrbl/v2/crud_builder.py  –  CRUD building functionality for Tigrbl.
...
def _invoke_all_registrars(model: type, api) -> None:
    """
    Call every distinct  __tigrbl_register_hooks__  found in the MRO,
    ...
    for base in reversed(model.__mro__):
        if "__tigrbl_register_hooks__" not in base.__dict__:
            continue
        if base in seen:
            continue
        print(f"Registering hooks from base {base}")
        raw = base.__dict__["__tigrbl_register_hooks__"]
