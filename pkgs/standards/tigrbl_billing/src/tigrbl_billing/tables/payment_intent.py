...
from tigrbl.types import (
    Mapped, String, Integer, JSONB, Boolean, SAEnum, UniqueConstraint, CheckConstraint, PgUUID, UUID)
...
class PaymentIntent(Base, GUIDPk, Timestamped):
    ...
    customer_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="customers.id"), nullable=True),
    ...
    connected_account_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="connected_accounts.id"), nullable=True),
            field=F(py_type=UUID | None),
            io=IO(in_verbs=("create","update","replace","merge"), out_verbs=("read","list")))
    )
    ...
    __table_args__ = (
        UniqueConstraint("stripe_payment_intent_id", name="uq_pi_stripe_payment_intent_id"),
        CheckConstraint("amount >= 0", name="ck_pi_amount_nonneg"),
        CheckConstraint("application_fee_amount IS NULL OR application_fee_amount >= 0", name="ck_pi_appfee_nonneg"))
