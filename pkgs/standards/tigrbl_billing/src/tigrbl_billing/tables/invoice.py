...
from tigrbl.types import (
    Mapped, String, Integer, JSONB, SAEnum, UniqueConstraint, CheckConstraint, PgUUID, UUID, TZDateTime)
...
class Invoice(Base, GUIDPk, Timestamped):
    ...
    customer_id: Mapped[UUID] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="customers.id"), nullable=False),
    ...
    subscription_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="subscriptions.id"), nullable=True),
    ...
    __table_args__ = (
        ...
        CheckConstraint("total >= 0 AND amount_due >= 0 AND amount_paid >= 0 AND amount_remaining >= 0",
