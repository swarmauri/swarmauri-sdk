...
class Subscription(Base, GUIDPk, Timestamped):
    ...
    collection_method: Mapped[CollectionMethod] = acol(
        spec=ColumnSpec(
            storage=S(type_=SAEnum, default=CollectionMethod.CHARGE_AUTOMATICALLY, nullable=False),
    ...
    __table_args__ = (
        UniqueConstraint("stripe_subscription_id", name="uq_subs_stripe_subscription_id"),
        CheckConstraint("current_period_end > current_period_start", name="ck_subs_period_valid"))
