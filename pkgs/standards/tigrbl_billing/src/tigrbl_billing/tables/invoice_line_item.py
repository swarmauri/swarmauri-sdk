...
from tigrbl.types import (
    Mapped, String, Integer, JSONB, Boolean, CheckConstraint, PgUUID, UUID)
...
class InvoiceLineItem(Base, GUIDPk, Timestamped):
    ...
    invoice_id: Mapped[UUID] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="invoices.id"), nullable=False),
    ...
    subscription_item_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="subscription_items.id"), nullable=True),
    ...
    price_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="prices.id"), nullable=True),
            field=F(py_type=UUID | None),
            io=IO(in_verbs=("create","update","replace","merge"), out_verbs=("read","list")))
    )
    ...
    __table_args__ = (
        CheckConstraint("amount >= 0 AND (unit_amount IS NULL OR unit_amount >= 0)", name="ck_ili_amounts_nonneg"))
