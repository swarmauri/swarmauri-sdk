...
class BalanceTopOff(Base, GUIDPk, Timestamped):
    ...
    balance_id: Mapped[UUID] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID, nullable=False, fk=ForeignKeySpec("customer_balances.id", ondelete="CASCADE"), index=True),
            field=F(py_type=str),
    ...
    status: Mapped[TopOffStatus] = acol(
        spec=ColumnSpec(
            storage=S(type_=SAEnum(TopOffStatus), nullable=False, default=TopOffStatus.INITIATED),
