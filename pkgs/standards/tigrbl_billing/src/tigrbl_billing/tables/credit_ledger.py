...
from tigrbl.types import Mapped, String, Integer, JSONB, SAEnum, UUID, PgUUID, TZDateTime
...
class CreditLedger(Base, GUIDPk, Timestamped):
    ...
    customer_id: Mapped[UUID] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID, nullable=False, fk=ForeignKeySpec("customers.id", ondelete="CASCADE"), index=True),
            field=F(py_type=str),
    ...
    balance_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID, nullable=True, fk=ForeignKeySpec("customer_balances.id", ondelete="SET NULL"), index=True),
    ...
    grant_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID, nullable=True, fk=ForeignKeySpec("credit_grants.id", ondelete="SET NULL"), index=True),
            field=F(py_type=str | None),
            io=IO(in_verbs=("create","update","replace"), out_verbs=("read","list")))
    )
    ...
    usage_event_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID, nullable=True, fk=ForeignKeySpec("usage_events.id", ondelete="SET NULL"), index=True),
            field=F(py_type=str | None),
            io=IO(in_verbs=("create","update","replace"), out_verbs=("read","list")))
    )
