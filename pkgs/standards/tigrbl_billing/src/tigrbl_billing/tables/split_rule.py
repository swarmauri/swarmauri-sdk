...
from tigrbl.types import (
    Mapped, String, Integer, JSONB, SAEnum, UniqueConstraint, CheckConstraint, PgUUID, UUID)
...
class SplitRule(Base, GUIDPk, Timestamped):
    ...
    application_fee_bps: Mapped[int | None] = acol(
        spec=ColumnSpec(
            ...
            field=F(py_type=int | None, constraints={"ge": 0, "le": 10000}),  # basis points (0..10000)
            io=IO(in_verbs=("create","update","replace","merge"), out_verbs=("read","list")))
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint("application_fee_bps IS NULL OR (application_fee_bps >= 0 AND application_fee_bps <= 10000)",
                        name="ck_split_rules_bps_range"),
        CheckConstraint("fixed_application_fee_amount IS NULL OR fixed_application_fee_amount >= 0",
