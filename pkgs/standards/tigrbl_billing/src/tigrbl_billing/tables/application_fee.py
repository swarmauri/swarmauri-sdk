...
from tigrbl.types import (
    Mapped, String, Integer, JSONB, Boolean, UniqueConstraint, CheckConstraint, PgUUID, UUID)
...
class ApplicationFee(Base, GUIDPk, Timestamped):
    ...
    originating_payment_intent_id: Mapped[UUID | None] = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="payment_intents.id"), nullable=True),
    ...
    __table_args__ = (
        UniqueConstraint("stripe_application_fee_id", name="uq_application_fees_stripe_id"),
