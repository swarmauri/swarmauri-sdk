...
def ingest_webhook_event(op_ctx, engine_ctx, schema_ctx, *, payload: dict | str, signature: Optional[str]=None, endpoint_secret: Optional[str]=None, account: Optional[str]=None, stripe_request_id: Optional[str]=None, api_version: Optional[str]=None, event_type: Optional[str]=None, event_created_ts: Optional[object]=None) -> dict:
    """
    ...
    with _session_for(StripeEventLog, op_ctx) as db:
        obj = None
        if hasattr(db, 'query'):
            obj = db.query(StripeEventLog).filter(StripeEventLog.stripe_event_id == stripe_event_id).one_or_none()
        if obj is None:
            obj = StripeEventLog(stripe_event_id=stripe_event_id, event_type=et, api_version=av, event_created_ts=event_created_ts, account=account, payload=data, status=EventProcessStatus.RECEIVED, processed_at=None, error_message=None, stripe_request_id=stripe_request_id)
        else:
            pass
        db.add(obj)
        db.flush()
