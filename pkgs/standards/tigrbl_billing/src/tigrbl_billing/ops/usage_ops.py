...
from tigrbl_billing.tables.usage_event import UsageEvent, UsageSource
...
def rollup_usage_periodic(op_ctx, engine_ctx, schema_ctx, *, subscription_item_id: UUID, period_start: object, period_end: object) -> dict:
    ...
    with _session_for(UsageRollup, op_ctx) as db:
        total = 0
        if hasattr(db, 'query'):
            q = db.query(UsageEvent).filter(UsageEvent.subscription_item_id == subscription_item_id)
            q = q.filter(UsageEvent.event_ts >= period_start, UsageEvent.event_ts < period_end)
            total = sum((e.quantity for e in q.all()))
        existing = None
        if hasattr(db, 'query'):
            existing = db.query(UsageRollup).filter(UsageRollup.subscription_item_id == subscription_item_id, UsageRollup.period_start == period_start, UsageRollup.period_end == period_end).one_or_none()
        if existing is None:
            r = UsageRollup(subscription_item_id=subscription_item_id, period_start=period_start, period_end=period_end, quantity_sum=total, last_event_ts=None)
        else:
            r = existing
