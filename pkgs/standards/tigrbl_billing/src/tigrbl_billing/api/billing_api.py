...
from tigrbl_billing.ops import (
    create_or_link_customer, attach_payment_method,
    start_subscription, cancel_subscription, pause_subscription, resume_subscription,
    trial_start, trial_end, proration_preview,
    seat_assign, seat_release, seat_suspend,
    rollup_usage_periodic,
    finalize_invoice, void_invoice, mark_invoice_uncollectible,
    capture_payment_intent, cancel_payment_intent,
    refund_application_fee,
    ingest_webhook_event, sync_objects)
...
@op_ctx(alias="attach_payment_method", target="custom", arity="collection", bind=Customer)
...
# -----------------------
@op_ctx(alias="refund_app_fee", target="custom", arity="collection", bind=ApplicationFee)
...
def build_billing_app(async_mode: bool = True) -> TigrblApp:
    ...
    app.include_models([
        ...
        # Seats & usage
        SeatAllocation, UsageEvent, UsageRollup,
        # Invoicing & payments
        Invoice, InvoiceLineItem, PaymentIntent, Refund,
        # Checkout
        CheckoutSession,
        # Connect
        ConnectedAccount, CustomerAccountLink, SplitRule, Transfer,
        # Webhooks & internal
        WebhookEndpoint, ApplicationFee, StripeEventLog,
    ])
    return app
...
# ---- Extensions: balances / grants / credit usage (no Upsertable) ----
...
from tigrbl_billing.ops import request_top_off, apply_top_off, check_and_top_off, apply_grant, revoke_grant, charge_credits

@op_ctx(alias="request_top_off", target="custom", arity="collection", bind=CustomerBalance)
...
@op_ctx(alias="check_and_top_off", target="custom", arity="collection", bind=CustomerBalance)
...
def __extend_app(app):
    app.include_models([CustomerBalance, BalanceTopOff, CreditGrant, CreditUsagePolicy, CreditLedger])
    return app
