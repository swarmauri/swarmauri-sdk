...
function cacheKeyFromManifest(manifest: LayoutManifest, explicit?: string): string {
  ...
  const version =
    manifest.meta?.atoms && typeof manifest.meta.atoms === "object"
...
export async function loadManifest(
  ...
  const registry: AtomRegistryMap = new Map();
  ...
  for (const atom of swarmaAtoms) {
    ...
    const component = (await loader(atom)) as LoadedComponent["component"];
    registry.set(atom.role, { component, atom });
  }
