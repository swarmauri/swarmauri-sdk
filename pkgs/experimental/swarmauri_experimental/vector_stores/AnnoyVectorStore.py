...
class AnnoyVectorStore(VectorStoreSaveLoadMixin, VectorStoreRetrieveMixin):
    """
    ...
    def __init__(self, dimension: int, metric="euclidean", num_trees=10):
        ...
        self.vectorizer = Doc2VecEmbedding()
        self.metric = metric
        ...
        VectorStoreSaveLoadMixin.__init__(self, self.vectorizer, [])
    ...
    def set_state(self, state: dict) -> None:
        ...
        """
        self.documents = [
            Document.from_dict(doc_dict) for doc_dict in state.get("documents", [])
        ]
            ...
            with open(temp_state_file.name, "rb") as src:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    with open(
                        os.path.join(directory_path, f"state_part_{part_num}.pkl"), "wb"
                    ) as dest:
                        dest.write(chunk)
                    part_num += 1
        finally:
        ...
        with open(index_file, "rb") as src:
            ...
            while True:
                ...
                if not chunk:
                    break
                with open(
                    os.path.join(directory_path, f"index_part_{part_num}.ann"), "wb"
                ) as dest:
                    dest.write(chunk)
                part_num += 1

    def load_parts(
        self, directory_path: str, state_file_pattern: str, index_file_pattern: str
    ) -> None:
        """
            ...
            with open(temp_state_file.name, "ab") as dest:
                ...
                while True:
                    part_file_path = os.path.join(
                        directory_path, state_file_pattern.format(part_num)
                    )
