...
try:
    ...
    class TigrblSessionBase(ABC):
        def __init__(self, spec=None): self._spec=spec; self._open=False; self._dirty=False
        async def begin(self): self._open=True
        async def commit(self): self._open=False; self._dirty=False
        async def rollback(self): self._open=False; self._dirty=False
        def in_transaction(self): return self._open
        async def run_sync(self, fn): rv=fn(self); return await rv if hasattr(rv,'__await__') else rv
        def apply_spec(self, spec): self._spec=spec
        ...
        def add(self, obj): self._dirty=True; return self._add_impl(obj)
        async def delete(self, obj): self._dirty=True; return await self._delete_impl(obj)
        async def flush(self): return await self._flush_impl()
        async def refresh(self, obj): return await self._refresh_impl(obj)
        async def get(self, model, ident): return await self._get_impl(model, ident)
        async def execute(self, stmt): return await self._execute_impl(stmt)
        async def close(self): return await self._close_impl()

try:
    from tigrbl.session.spec import SessionSpec
except Exception:
    class SessionSpec:
        def __init__(self, isolation=None, read_only=None): self.isolation=isolation; self.read_only=read_only

try:
    from tigrbl.core.crud.helpers.model import _single_pk_name, _model_columns
except Exception:
    # Minimal fallbacks
    def _single_pk_name(model): return "id"
    def _model_columns(model): return getattr(model, "__annotations__", {}) or {"id": int}
...
class _ScalarResult:
    ...
    def scalars(self) -> "_ScalarResult": return self
    def all(self) -> List[Any]: return list(self._items)
...
class TransactionalDataFrameSession(TigrblSessionBase):
    ...
    def __init__(self, catalog: DataFrameCatalog, spec: Optional[SessionSpec] = None) -> None:
        super().__init__(spec)
        self._cat = catalog
    ...
    async def _tx_begin_impl(self) -> None:
        self._snap.clear(); self._snap_ver.clear()
        self._puts.clear(); self._dels.clear()
    ...
    def _frame_for(self, model: type) -> pd.DataFrame:
        ...
        iso = (self._spec.isolation if self._spec else None) or "read_committed"
        if iso in ("repeatable_read", "snapshot", "serializable") and table not in self._snap:
            live = self._cat.get_live(table)
            self._snap[table] = live.copy(deep=True)
            self._snap_ver[table] = self._cat.table_ver.get(table, 0)
        return self._snap.get(table) if table in self._snap else self._cat.get_live(table)
    ...
    def _decompose_select(self, stmt: Any) -> Tuple[type, list[Tuple[str, str, Any]], list[Tuple[str, str]], Optional[int], Optional[int]]:
    ...
    def _extract_model(self, stmt: Any) -> type:
        ...
        if rc:
            ent = rc[0]; table = getattr(ent, "table", None); name = getattr(table, "name", None)
            if name:
                for cls in object.__subclasses__(object):
                    if getattr(cls, "__tablename__", None) == name:
                        return cls
    ...
    def _extract_predicates(self, stmt: Any) -> list[Tuple[str, str, Any]]:
        where = getattr(stmt, "whereclause", None) or getattr(stmt, "_whereclause", None)
        if where is None:
            return []
        parts = getattr(where, "clauses", None) or getattr(where, "get_children", lambda: [])()
        nodes = list(parts) if parts else [where]
        out: list[Tuple[str, str, Any]] = []
        for n in nodes:
            left, right, op = getattr(n, "left", None), getattr(n, "right", None), getattr(n, "operator", None)
            if left is None or right is None: continue
            name = getattr(left, "key", None) or getattr(left, "name", None)
            if name is None: continue
            opname = getattr(op, "__name__", str(op))
            if "eq" in opname:
                val = getattr(right, "value", None) if hasattr(right, "value") else getattr(right, "literal", None)
                out.append((str(name), "eq", val)); continue
            rclauses = getattr(right, "clauses", None)
            if rclauses is not None and "in" in opname:
                vals = [getattr(lit, "value", None) if hasattr(lit, "value") else getattr(lit, "literal", None) for lit in rclauses]
                out.append((str(name), "in", vals))
        return out

    def _extract_order_by(self, stmt: Any) -> list[Tuple[str, str]]:
        order = getattr(stmt, "_order_by_clause", None) or getattr(stmt, "_order_by_clauses", None)
        if not order: return []
        clauses = getattr(order, "clauses", None) or order
        clauses = clauses if isinstance(clauses, (list, tuple)) else [clauses]
        for ob in clauses:
            col = getattr(ob, "element", None) or getattr(ob, "this", None) or getattr(ob, "expr", None)
            name = getattr(col, "key", None) or getattr(col, "name", None)
            direction = "desc" if "desc" in type(ob).__name__.lower() else "asc"
            if name: return [(str(name), direction)]
        return []
    ...
    def _matches_obj(self, obj: Any, where: list[Tuple[str, str, Any]]) -> bool:
        for name, op, val in where:
            ...
            if op == "eq" and dv != val: return False
            if op == "in" and dv not in set(val): return False
        return True

    def _order_slice(self, items: List[Any], order: list[Tuple[str, str]], limit: Optional[int], offset: Optional[int]) -> List[Any]:
        if order:
            col, direction = order[0]
            items.sort(key=lambda o: getattr(o, col, None), reverse=(direction == "desc"))
        if isinstance(offset, int): items = items[max(0, offset):]
        if isinstance(limit, int): items = items[: max(0, limit)]
        return items
