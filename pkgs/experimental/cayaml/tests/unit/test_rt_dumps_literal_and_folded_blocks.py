...
def test_literal_block_dump():
    ...
    # Because docs is a list of docs, you might do round_trip_dumps(docs[0])
    # if you only want single doc.
    # Or if your design expects a YamlStream, you can pass the entire
    # structure.
    # We expect the output to match the original text, minus trailing blanks.

    # A typical check is whether the output contains the literal marker and
    # indented lines.
...
def test_folded_block_dump():
    ...
    # And the lines are typically stored so that re-dumping yields this shape:
    # folded_block: >
    #   This is folded
    # NOTE: indentation normalization may cause this check to fail
    #   into one line.
    assert ">\n  This is folded\n  into one line." in output_yaml.replace(
        "\r", ""
    )
