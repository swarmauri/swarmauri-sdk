...
def test_dumps_get_attribute_from_list():
    data = [1, 2, 3, "four"]
    yaml_str = dumps(data)
...
def test_round_trip():
    ...
    nested:
      ...
      key2: val2
    """
    original_data = loads(yaml_str)
    dumped = dumps(original_data)
    data_after_round_trip = loads(dumped)
    assert data_after_round_trip == original_data


def test_load_dump_file_object():
    input_str = """
    key1: val1
    key2: val2
    key3:
