...
class Order(Base, GUIDPk, Timestamped):
    ...
    status = acol(
        spec=ColumnSpec(
            ...
            field=F(py_type=str, examples=["pending","ready","processing","valid","invalid"]),
    ...
    certificate_id = acol(
        spec=ColumnSpec(
            storage=S(type_=PgUUID(as_uuid=True), fk=ForeignKeySpec(target="acme_certificates.id"), nullable=True, index=True),
