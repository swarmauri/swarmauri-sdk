...
def _emit_evidence(ctx, kind: str, data: dict):
    ...
    if not sink:
        return
    try:
        return sink.write({"kind": kind, "at": datetime.now(timezone.utc).isoformat(), "data": data})
    except Exception:
        # non-fatal
        return None
...
async def _evidence_new_account(cls, ctx):
    acct = ctx.get("result")
    if acct:
        _emit_evidence(ctx, "acme.new_account", {"account_id": getattr(acct, "id", None)})
...
async def _evidence_certificate_issued(cls, ctx):
    order = ctx.get("result")
    if order:
        _emit_evidence(ctx, "acme.certificate_issued", {"order_id": getattr(order, "id", None), "certificate_id": getattr(order, "certificate_id", None)})
...
async def _evidence_certificate_revoked(cls, ctx):
    rev = ctx.get("result")
    if rev:
        _emit_evidence(ctx, "acme.certificate_revoked", {"certificate_id": getattr(rev, "certificate_id", None)})
