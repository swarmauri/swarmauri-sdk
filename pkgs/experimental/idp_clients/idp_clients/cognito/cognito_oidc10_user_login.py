...
class CognitoOIDC10Login:
    ...
    async def exchange(self, code: str, state: str) -> Dict[str, Any]:
        ...
        if not idt: raise ValueError("no id_token in response")
        header = jwt.get_unverified_header(idt)
        key = next((k for k in jwks["keys"] if k["kid"] == header.get("kid")), None)
        if not key: raise ValueError("signing key not found")
        ...
        return {"issuer":"cognito-oidc", "sub": claims["sub"], "email": email, "name": name, "raw": {"tokens": tok}}
