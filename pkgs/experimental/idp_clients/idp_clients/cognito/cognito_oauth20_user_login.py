...
class CognitoOAuth20Login:
    ...
    async def exchange_and_userinfo(self, code: str, state: str) -> Dict[str, Any]:
        ...
        async with RetryingAsyncClient() as c:
            ...
            ui  = (await c.get_retry(d.get("userinfo_endpoint", f"{self.issuer.rstrip('/')}/oauth2/userInfo"), headers=hdr)).json()
        return {"issuer":"cognito-oauth2", "sub": ui.get("sub"),
                "email": ui.get("email"), "name": ui.get("name") or ui.get("given_name") or "Unknown",
