...
class GiteaOIDC10Login:
    """OpenID Connect 1.0 Authorization Code + PKCE for Gitea (if OIDC provider is enabled)."""
    issuer: str
    client_id: str
    client_secret: str
    redirect_uri: str
    ...
    async def exchange(self, code: str, state: str) -> Dict[str, Any]:
        ...
        return {"issuer":"gitea-oidc", "sub": claims["sub"], "email": email, "name": name, "raw": {"tokens": tok}}
