...
class AppleOAuth21Login:
    """OAuth 2.1-aligned Authorization Code + PKCE for Apple; verifies ID token (OIDC)."""
    team_id: str
    key_id: str
    client_id: str
    ...
    async def exchange_and_identity(self, code: str, state: str) -> Dict[str, Any]:
        ...
        return {"issuer":"apple-oauth21", "sub": claims["sub"], "email": claims.get("email"), "name": claims.get("name")}
