...
class GitLabOIDC10Login:
    \"\"OpenID Connect 1.0 Authorization Code + PKCE for GitLab.
    issuer: GitLab OIDC issuer (SaaS or self-managed), e.g. https://gitlab.com
    \"\"
    issuer: str
    client_id: str
    client_secret: str
    ...
    async def exchange(self, code: str, state: str) -> Dict[str, Any]:
        ...
        # Minimal identity; fallback to UserInfo if email missing
        ...
        return {"issuer":"gitlab-oidc", "sub": claims["sub"], "email": email, "name": name, "raw": {"tokens": tok}}
