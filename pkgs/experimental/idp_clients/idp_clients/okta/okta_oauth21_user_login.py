...
class OktaOAuth21Login:
    """OAuth 2.1-aligned Authorization Code + PKCE for Okta.
    We still request OIDC identity scopes and verify ID token for login-only identity.
    """
    issuer: str
    client_id: str
    client_secret: str
    ...
    async def exchange_and_identity(self, code: str, state: str) -> Dict[str, Any]:
        ...
        return {"issuer":"okta-oauth21", "sub": claims["sub"],
