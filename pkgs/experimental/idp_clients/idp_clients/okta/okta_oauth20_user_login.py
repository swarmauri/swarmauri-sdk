...
class OktaOAuth20Login:
    """OAuth 2.0 Authorization Code + PKCE for Okta using the UserInfo endpoint for identity."""
    issuer: str
    client_id: str
    client_secret: str
    redirect_uri: str
    state_secret: bytes
    scope: str = "openid email profile"   # provides access to userinfo
    ...
    async def exchange_and_userinfo(self, code: str, state: str) -> Dict[str, Any]:
        ...
        return {"issuer":"okta-oauth2", "sub": ui.get("sub") or ui.get("uid") or ui.get("sub_id"),
