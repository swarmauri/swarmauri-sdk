...
class HTTP2Multiplexer:
    ...
    def create_stream(self):
        ...
        self.next_stream_id += (
            2  # Increment by 2 (client-initiated streams must be odd).
        )
    ...
    async def send_request(self, headers, data=b""):
        ...
        headers_frame = HTTP2Frame(
            frame_type=0x1, flags=0x4, stream_id=stream_id, payload=header_block
        )
        ...
        if data:
            ...
            data_frame = HTTP2Frame(
                frame_type=0x0, flags=0x1, stream_id=stream_id, payload=data
            )
