...
def test_functionality_of_cpp_extension() -> None:
    ...
    import DistanceMetrics.extensions  # Replace with the actual name of your C++ extension module
    result = DistanceMetrics.extensions.example_function()  # Replace with an actual function
...
def test_numpy_integration() -> None:
    ...
    result = DistanceMetrics.extensions.numpy_function(array)  # Replace with an actual function that uses NumPy
    assert isinstance(result, np.ndarray), "The result should be a NumPy array."
    assert result.shape == array.shape, "The shape of the result should match the input array."