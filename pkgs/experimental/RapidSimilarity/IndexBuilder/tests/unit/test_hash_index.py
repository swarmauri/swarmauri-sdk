...
def test_lsh_index_insert_and_query():
    ...
    assert all(np.array_equal(result, data_point) for result in results), "Returned results should match the inserted data point."
...
def test_lsh_index_bulk_insert_and_query(data_points):
    ...
    assert any(np.array_equal(result, data_points[0]) for result in results), "Returned results should contain the queried data point."
...
def test_invalid_insert():
    ...
    with pytest.raises(ValueError):
        lsh_index.insert(np.array([], dtype=np.float32))  # Should raise ValueError for empty input
