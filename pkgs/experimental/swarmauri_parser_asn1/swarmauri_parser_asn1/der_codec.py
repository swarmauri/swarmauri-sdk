...
def _expect_tag(
    buf_tag: Tuple[int, bool, int], exp: Tag | None, builtin_kind: str | None
):
    ...
    if builtin_kind:
        if cls != 0:
            return False
        if tag != _UNIVERSAL_TAGS[builtin_kind]:
            return False
        return True
    return True
...
def _dec_octet_string(v: bytes) -> bytes:
    return v


def _dec_utf8(v: bytes) -> str:
    return v.decode("utf-8")


def _dec_ia5(v: bytes) -> str:
    return v.decode("ascii")


def _dec_printable(v: bytes) -> str:
    return v.decode("ascii")
...
def decode_value(schema: Schema, tnode: Any, buf: bytes, i: int = 0):
    ...
    if isinstance(node, Builtin):
        ...
        if not _expect_tag(
            (cls, cons, tag),
            tagwrap if (tagwrap and tagwrap.mode == "implicit") else None,
            node.kind,
        ):
            raise ValueError(f"Tag mismatch for {node.kind}")
        kind = node.kind
        if kind == "BOOLEAN":
            return _dec_boolean(val), end
        if kind == "INTEGER":
            x = _dec_integer(val)
            if node.enum:
        ...
        if kind == "OCTET STRING":
            return _dec_octet_string(val), end
        if kind == "UTF8String":
            return _dec_utf8(val), end
        if kind == "IA5String":
            return _dec_ia5(val), end
        if kind == "PrintableString":
            return _dec_printable(val), end
        if kind == "NULL":
            return None, end
