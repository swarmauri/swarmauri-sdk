...
def test_jupyter_export_html_tool_call_with_template(valid_notebook_json: str) -> None:
    ...
    tool = JupyterExportHTMLTool()
    result = tool(
        notebook_json=valid_notebook_json,
        template_file="dummy_template.tpl"
    )
    assert isinstance(result, dict), "Result should be a dictionary."
...
def test_jupyter_export_html_tool_call_with_extra_css(valid_notebook_json: str) -> None:
    ...
    assert "exported_html" in result, "The result should contain 'exported_html'."
    assert css_content in result["exported_html"], "Extra CSS should be present in the exported HTML."
...
def test_jupyter_export_html_tool_call_with_extra_js(valid_notebook_json: str) -> None:
    ...
    assert "exported_html" in result, "The result should contain 'exported_html'."
    assert js_content in result["exported_html"], "Extra JS should be present in the exported HTML."

def test_jupyter_export_html_tool_failure_with_invalid_json(invalid_notebook_json: str) -> None:
    """
    Tests that providing invalid JSON triggers an error during export.
    """
    tool = JupyterExportHTMLTool()
    result = tool(notebook_json=invalid_notebook_json)
    assert "error" in result, "The result should contain an 'error' key if notebook JSON is invalid."
    assert "An error occurred" in result["error"], "The error message should indicate that an error occurred."
