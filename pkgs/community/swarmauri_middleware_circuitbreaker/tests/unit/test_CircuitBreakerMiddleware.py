...
from swarmauri_middleware_circuitbreaker.CircuitBreakerMiddleware import CircuitBreakerMiddleware
import logging
...
class TestCircuitBreakerMiddleware:
    ...
    def successful_call_next(self, mocker):
        ...
        return mocker.asyncio_mock(asyncfunction=mock_call_next)
    ...
    async def test_circuit_state_changes(self, middleware, caplog, failing_call_next, successful_call_next):
    ...
    def test_circuit_breaker_parameters(self):
        """Test circuit breaker parameters are correctly set."""
        fail_max = 10
        reset_timeout = 60
        half_open_wait_time = 15
        
        custom_middleware = CircuitBreakerMiddleware(
            fail_max=fail_max,
            reset_timeout=reset_timeout,
            half_open_wait_time=half_open_wait_time
        )
        
        assert custom_middleware.circuit_breaker.fail_max == fail_max
        assert custom_middleware.circuit_breaker.reset_timeout == reset_timeout
        assert custom_middleware.circuit_breaker.half_open_wait_time == half_open_wait_time

    async def test_circuit_recovery(self, middleware, failing_call_next, successful_call_next):
