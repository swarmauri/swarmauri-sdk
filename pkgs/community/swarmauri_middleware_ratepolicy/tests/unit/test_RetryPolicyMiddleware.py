...
class TestRetryPolicyMiddleware:
    ...
    @pytest.mark.parametrize("max_retries,initial_wait", [
        (0, 1),
        (10, 5),
    ...
    @pytest.mark.parametrize("max_retries,expected_retries", [
        (3, 3),
        (5, 5),
        (0, 0)
    ])
    ...
    def test_call_magic_method(self, mocker):
        ...
        dispatch_mock = mocker.patch.object(
            middleware,
            'dispatch',
            return_value="Dispatch Response"
        )
    ...
    @pytest.mark.parametrize("max_retries,expected_type", [
        (3, RetryPolicyMiddleware),
        (0, RetryPolicyMiddleware)
    ])
