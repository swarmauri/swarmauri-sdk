...
def test_tool_initialization() -> None:
    ...
    assert tool.name == "JupyterStartKernelTool", "Tool name should match expected default."
    assert tool.version == "1.0.0", "Default version should be '1.0.0'."
    assert tool.type == "JupyterStartKernelTool", "Tool type should be 'JupyterStartKernelTool'."
    assert len(tool.parameters) == 2, "Expected two default parameters (kernel_name, kernel_spec)."
...
@patch("swarmauri_tool_jupyterstartkernel.JupyterStartKernelTool.KernelManager", autospec=True)
def test_call_success(
    ...
    assert result["kernel_name"] == expected_kernel_name, "Kernel name should match the input."
    assert result["kernel_id"] == "fake_kernel_id", "Mock kernel ID should match the returned value."
...
def test_call_failure(mock_kernel_manager_class: MagicMock) -> None:
    ...
    """
    mock_kernel_manager = MagicMock()
    mock_kernel_manager.start_kernel.side_effect = RuntimeError("Failed to start kernel.")
    mock_kernel_manager_class.return_value = mock_kernel_manager
...
def test_tool_parameters() -> None:
    ...
    assert tool.parameters[0]["name"] == "kernel_name", "First parameter should be kernel_name."
    assert tool.parameters[1]["name"] == "extra_config", "Second parameter should be extra_config."
    assert tool.parameters[1]["default"] == {}, "Default for extra_config should be an empty dict."
...
def test_call_with_kernel_spec() -> None:
    ...
    """
    tool = JupyterStartKernelTool()
    result: Dict[str, Any] = tool(kernel_name="python3", kernel_spec={"env": {"KEY": "VALUE"}})
    ...
    if "error" in result:
        ...
        assert "kernel_name" in result, "A successful call should return a 'kernel_name'."