...
def test_tool_metadata(tool_instance: JupyterExportMarkdownTool) -> None:
    ...
    assert tool_instance.description == "Converts a Jupyter Notebook into Markdown format."
    assert tool_instance.version == "1.0.0"
    assert tool_instance.type == "JupyterExportMarkdownTool"


def test_export_basic_notebook(tool_instance: JupyterExportMarkdownTool, sample_notebook_json: Dict[str, Any]) -> None:
    """
    ...
    assert "exported_markdown" in result, "Expected 'exported_markdown' in the return dictionary."
    assert "Notebook" in result["exported_markdown"], "Expected the heading from the sample notebook in the output."


def test_export_with_styles(tool_instance: JupyterExportMarkdownTool, sample_notebook_json: Dict[str, Any]) -> None:
    """
    ...
    assert "error" not in result, f"Unexpected error encountered: {result.get('error', '')}"


def test_export_with_template(tool_instance: JupyterExportMarkdownTool, sample_notebook_json: Dict[str, Any]) -> None:
    """
...
def test_export_error_handling(tool_instance: JupyterExportMarkdownTool) -> None:
    ...
    """
    invalid_notebook_json: Optional[Dict[str, Any]] = None  # Invalid type for the notebook structure
    result = tool_instance(notebook_json=invalid_notebook_json)  # type: ignore[arg-type]
    assert "error" in result, "Expected 'error' key in the return dictionary for invalid input."
    assert "Failed to export notebook" in result["error"], "Expected an error message indicating a failure."