...
@pytest.mark.parametrize(
    ...
    [
        (
            (
                "This is a sample text with some complex sentences and "
                "polysyllabic words to test the SMOG Index calculation."
            ),
            8.5,
        ...
        (
            (
                "A longer piece of text that is still relatively simple but "
                "includes a few more sentences and polysyllabic words for a "
...
def test_call(input_text, expected_smog_index):
    ...
    assert result.get("smog_index") == pytest.approx(
        ...
        f"Expected SMOG Index {pytest.approx(expected_smog_index_calculated, rel=0.1)}, but got {result.get('smog_index')}"  # noqa: E501
    )
