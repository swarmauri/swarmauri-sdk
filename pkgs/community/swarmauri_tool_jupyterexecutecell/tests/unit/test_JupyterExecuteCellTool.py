...
def test_tool_initialization():
    ...
    assert tool.description == "Executes code cells within a Jupyter kernel environment."
    assert tool.type == "JupyterExecuteCellTool", "Tool type should be 'JupyterExecuteCellTool'."
    assert len(tool.parameters) == 2, "There should be two default parameters: code, timeout."
...
def test_tool_call_basic_execution():
    ...
    assert "Hello, world!" in result["stdout"], "Expected code execution output not found in stdout."
...
def test_tool_call_syntax_error():
    ...
    assert "SyntaxError" in result["error"], "Expected a SyntaxError in the error field."
...
def test_tool_call_timeout():
    ...
    assert "Execution timed out after 1 seconds." in result["error"], "Expected timeout error message."
    assert result["stdout"] == "", "stdout should be empty on timeout."
    assert result["stderr"] == "", "stderr should be empty on timeout."

@patch("swarmauri_tool_jupyterexecutecell.JupyterExecuteCellTool.get_ipython", return_value=None)
def test_tool_call_no_active_kernel(mock_ipython):
    ...
    assert result["stderr"] == "No active IPython kernel found.", "Expected stderr to mention no active kernel."
    assert result["error"] == "KernelNotFoundError", "Expected error to be 'KernelNotFoundError'."
...
def test_tool_call_exception_during_execution(mock_ipython):
    ...
    assert "Mocked runtime error" in result["error"], "Expected mocked runtime error in the error field."
    assert "RuntimeError" in result["error"], "Expected 'RuntimeError' text in error field."
    assert result["stderr"] != "", "stderr should capture exception details."
    assert "Testing exception" not in result["stdout"], "stdout should not have content from failing command."