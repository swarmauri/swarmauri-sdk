...
def ensure_kernel(kernel_name, display_name=None):
    ...
    subprocess.run(["python", "-m", "ipykernel", "install", "--user", "--name", kernel_name, "--display-name", display_name], check=True)
...
def main():
    ...
    if not is_kernel_available(required_kernel):
        raise RuntimeError(f"Kernel '{required_kernel}' is not available. Please install it with 'python -m ipykernel install --user --name {required_kernel}'")
