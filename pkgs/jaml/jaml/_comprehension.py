...
def _evaluate_comprehension(node, global_env, context=None):
    ...
    logger.debug(f'[_evaluate_comprehension] entering: node={node}, global_env_keys={list(global_env.keys())}, context={context}')
    results = []
    for env in iter_environments(node.clauses, global_env, {}, context or {}):
        ...
        if hasattr(node, 'header_expr'):
            logger.debug(f'[_evaluate_comprehension] resolving header_expr: {node.header_expr}')
            node.header_expr.resolve(global_env, env)
            logger.debug(f'[_evaluate_comprehension] resolved header_expr.resolved={node.header_expr.resolved}')
            ...
            logger.debug(f'[_evaluate_comprehension] resolved pair -> key={k}, value={v}')
            results.append((k, v))
    # for dict comprehensions, you’d turn results into a dict
    if isinstance(node, ListComprehensionNode):
        logger.debug(f'[_evaluate_comprehension] returning list of length {len(results)}')
        return results
    else:
        result_dict = dict(results)
        logger.debug(f'[_evaluate_comprehension] returning dict of size {len(result_dict)}')
        return result_dict
