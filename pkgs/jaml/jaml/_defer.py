...
def substitute_deferred(ast_node, env):
    """
    Traverse the AST recursively. For any DeferredExpression node, call its evaluate() 
    method with the provided environment and replace it with the evaluated result.
    ...
    if isinstance(ast_node, DeferredExpression):
        ...
        s = ast_node.value
        return re.sub(r'\$\{([^}]+)\}', lambda m: str(resolve_scoped_variable(m.group(1).strip(), env) or m.group(0)), s)
    elif isinstance(ast_node, str):
        return re.sub(r'\$\{([^}]+)\}', lambda m: str(resolve_scoped_variable(m.group(1).strip(), env) or m.group(0)), ast_node)
    else:
        return ast_node
