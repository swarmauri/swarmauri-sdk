...
c = f"%{a}"
    ...
    with pytest.raises(ValueError, match="Circular reference detected: config.a -> config.b -> config.nested.c -> config.a"):
        ast = round_trip_loads(jml)
...
z = f"%{x}"
    ...
    with pytest.raises(ValueError, match="Circular reference detected: x -> y -> z -> x"):
        ast = round_trip_loads(jml)
