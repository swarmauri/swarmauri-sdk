...
def test_round_trip_multiline_array_preserves_format():
    ...
    assert array_end != -1, "Could not find the closing ']' for the dev array in unparsed text"
    
    array_content = unparsed_text[array_start:array_end]
    # Assert that there is at least one newline inside the array content.
    assert "\n" in array_content, "Multiline array formatting (newlines) was not preserved in unparsed text"
...
def test_plain_loads_multiline_array():
    ...
    assert plain_data == expected, f"Expected plain data {expected}, but got {plain_data}"
