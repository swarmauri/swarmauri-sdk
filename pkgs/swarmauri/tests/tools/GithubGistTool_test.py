...
import pytest
from swarmauri.community.tools.concrete.GithubGistTool import GithubGistTool as Tool
...
def test_ubc_type():
    token = os.getenv("GITHUBTOOL_TEST_TOKEN")
    assert Tool(token=token).type == "GithubGistTool"
...
@pytest.mark.unit
@patch("swarmauri.community.tools.concrete.GithubGistTool.Github")
