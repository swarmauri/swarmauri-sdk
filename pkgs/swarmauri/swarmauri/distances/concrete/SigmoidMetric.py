...
class SigmoidMetric(DistanceBase):
    """
    Concrete implementation of the IDistanceSimilarity interface using the
    Sigmoid Metric.
    """
    type: Literal['SigmoidMetric'] = 'SigmoidMetric'

    def distance(self, vector_a: Vector, vector_b: Vector) -> float:
        """
        Computes the Sigmoid Metric distance between two vectors.
        ...
        distances = 1 / (1 + np.exp(-np.abs(data_a - data_b)))
    ...
    def similarity(self, vector_a: Vector, vector_b: Vector) -> float:
        """
        Computes a similarity score derived from the Sigmoid Metric.
