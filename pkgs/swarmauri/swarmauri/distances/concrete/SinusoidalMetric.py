...
class SinusoidalMetric(DistanceBase):
    """
    Concrete implementation of the IDistanceSimilarity interface using the
    Sinusoidal Metric.
    """
    type: Literal['SinusoidalMetric'] = 'SinusoidalMetric'

    def distance(self, vector_a: Vector, vector_b: Vector) -> float:
        """
        Computes the Sinusoidal Metric distance between two vectors.
        ...
        distances = np.sin(np.abs(data_a - data_b))
    ...
    def similarity(self, vector_a: Vector, vector_b: Vector) -> float:
        """
        Computes a similarity score derived from the Sinusoidal Metric.
