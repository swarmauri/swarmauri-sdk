...
def _detect_time_based_media(pdf_bytes: bytes) -> tuple[bool, dict]:
    flags = {
        ...
        "EmbeddedAudio": bool(
            _EMBEDDED_FILE_RE.search(pdf_bytes) and _EMBEDDED_SUBTYPE_AUDIO.search(pdf_bytes)
        ),
        "EmbeddedVideo": bool(
            _EMBEDDED_FILE_RE.search(pdf_bytes) and _EMBEDDED_SUBTYPE_VIDEO.search(pdf_bytes)
        ),
    }
...
def evaluate_sc(
    sc_num: str, title: str, level: str, pdf_paths: list[str], applicability: str, notes: str
) -> SCResult:
    ...
    if sc_num.startswith("1.1."):
        ...
        if total_images == 0:
            return SCResult(
                True, "No raster images detected; criterion not applicable for text-only PDFs."
            )
        alt_count = sum(_detect_alt_entries(b) for b in pdf_bytes_list)
        if alt_count >= 1:
            return SCResult(
                True,
                f"Detected images ({total_images}) with Alt text entries present (>= {alt_count}).",
            )
        return SCResult(
            False, f"Detected {total_images} images but no Alt text entries were found."
        )
    ...
    if sc_num.startswith("1.2."):
        ...
        return SCResult(
            False,
            (
                f"Detected {kinds}; specific alternatives "
                "(captions/descriptions) not verified in this phase."
            ),
        )
    ...
    if sc_num.startswith("1.3."):
        if sc_num == "1.3.1":
            if any(_is_tagged_pdf(b) for b in pdf_bytes_list):
                return SCResult(
                    True, "Tagged PDF detected (/StructTreeRoot or /MarkInfo /Marked true)."
                )
            total_images = sum(_detect_images(b) for b in pdf_bytes_list)
            ...
            if total_images == 0 and not any_media and not any_forms:
                return SCResult(
                    True, "Heuristic pass: simple text-only PDF without complex structures."
                )
            return SCResult(
                False, "No tagging detected and non-text structures present; add structural tags."
            )
        if sc_num == "1.3.2":
            if any(_is_tagged_pdf(b) for b in pdf_bytes_list):
            ...
            if total_images == 0 and not any_media and not any_forms:
                return SCResult(
                    True,
                    "Heuristic pass: simple text-only PDF—reading order unlikely to be ambiguous.",
                )
            return SCResult(
                False, "No tagging detected for content that may require explicit reading order."
            )
        if sc_num == "1.3.3":
            return SCResult(
                True,
                (
                    "Automated detection of sensory-only instructions not "
                    "applicable; no issues detected heuristically."
                ),
            )
        if sc_num == "1.3.4":
            return SCResult(
                True,
                (
                    "PDF content is not viewport-orientation restricted; "
                    "criterion treated as not applicable."
                ),
            )
        if sc_num == "1.3.5":
            if any(_has_acroform(b) for b in pdf_bytes_list):
                return SCResult(
                    False,
                    (
                        "Form fields detected; identifying input purpose "
                        "requires semantic mapping (not implemented)."
                    ),
                )
        ...
        if sc_num == "1.3.6":
            return SCResult(
                True,
                (
                    "Heuristic pass: identifying purpose at component level "
                    "is not applicable to static PDFs."
                ),
            )
    ...
    if sc_num.startswith("1.4."):
        ...
        if sc_num == "1.4.1":
            return SCResult(
                True,
                "Heuristic pass: no automated evidence of color-only instructions in static PDFs.",
            )
        # Audio Control
        if sc_num == "1.4.2":
            any_audio = False
        ...
        if sc_num == "1.4.3":
            ...
            if not (rgb or cmyk):
                return SCResult(
                    True, "No explicit color operators detected; likely black-on-white text."
                )
            if _only_near_black(rgb, gray, cmyk):
                return SCResult(
                    True, "Only near-black colors detected for drawing operations (heuristic pass)."
                )
            return SCResult(
                True,
                (
                    "Non-black colors detected; contrast not computed "
                    "automatically—manual review recommended."
                ),
            )
        # Resize Text
        if sc_num == "1.4.4":
            return SCResult(
                True, "PDF viewers support zoom; criterion treated as satisfied for static content."
            )
        # Images of Text (AA)
        if sc_num == "1.4.5":
            total_images = sum(_detect_images(b) for b in pdf_bytes_list)
            ...
            return SCResult(
                False,
                f"{total_images} images detected; cannot confirm they are not images of text.",
            )
        ...
        if sc_num == "1.4.6":
            ...
            if not (rgb or cmyk):
                return SCResult(
                    True,
                    "No explicit color operators detected; likely black-on-white text (enhanced).",
                )
            if _only_near_black(rgb, gray, cmyk):
                return SCResult(
                    True, "Only near-black colors detected for drawing operations (heuristic pass)."
            ...
            return SCResult(
                True,
                (
                    "Non-black colors detected; enhanced contrast not "
                    "computed automatically—manual review recommended."
                ),
            )
        ...
        if sc_num == "1.4.7":
            ...
            if any_audio:
                return SCResult(
                    False, "Background audio detected; low/no background audio not verified."
                )
            return SCResult(True, "No audio detected; criterion not applicable.")
        ...
        if sc_num == "1.4.8":
            return SCResult(
                True,
                (
                    "Heuristic pass: visual presentation constraints not "
                    "applicable to static PDF text without CSS."
                ),
            )
        ...
        if sc_num == "1.4.9":
            ...
            return SCResult(
                False,
                (
                    f"{total_images} images detected; cannot confirm they are not "
                    "images of text (AAA)."
                ),
            )
        # Reflow (AA)
        if sc_num == "1.4.10":
            # Heuristic: treat static, single-page text PDFs as acceptable.
            # Complex multi-column/layout is not verified.
            return SCResult(
                True,
                (
                    "Heuristic pass: PDF content expected to reflow via "
                    "viewer/zoom; complex layouts not detected."
                ),
            )
        # Non-text Contrast (AA)
        if sc_num == "1.4.11":
            # Heuristic: without vector shape analysis, assume non-text
            # contrast adequate when colors are near black.
            rgb, gray, cmyk = [], [], []
            for b in pdf_bytes_list:
                r, g, cm = _colors_used(b)
                rgb += r
            ...
            if not (rgb or cmyk) or _only_near_black(rgb, gray, cmyk):
                return SCResult(
                    True, "Heuristic pass: no evidence of low-contrast non-text elements."
                )
            return SCResult(
                True, "Non-text contrast not computed automatically—manual review recommended."
            )
        # Text Spacing (AA)
        if sc_num == "1.4.12":
            return SCResult(
                True,
                (
                    "Heuristic pass: user agents handle text spacing "
                    "adjustments; static PDF not script-driven."
                ),
            )
        ...
        if sc_num == "1.4.13":
            ...
            if js_present or interactive:
                return SCResult(
                    True,
                    (
                        "Heuristic pass: hover/focus-only content not "
                        "auto-verified in PDFs; no blocking evidence."
                    ),
                )
    ...
    if sc_num.startswith("2.1."):
        ...
        if sc_num == "2.1.1":
            ...
            return SCResult(
                False,
                "Interactive content detected; keyboard operability not verified automatically.",
            )
        if sc_num == "2.1.2":
            if not interactive:
                return SCResult(
                    True, "No interactive widgets detected; keyboard trap not applicable."
                )
            return SCResult(
                False, "Interactive content detected; cannot verify absence of keyboard traps."
            )
        if sc_num == "2.1.3":
            if not interactive:
                return SCResult(
                    True, "No interactive widgets detected; criterion not applicable (AAA)."
                )
            return SCResult(
                False,
                (
                    "Interactive content detected; keyboard-only operation "
                    "without exception not verified."
                ),
            )
        if sc_num == "2.1.4":
            js_present = any(_has_javascript(b) for b in pdf_bytes_list)
            if not js_present:
                return SCResult(
                    True, "No PDF JavaScript detected; no character key shortcuts applicable."
                )
    ...
    if sc_num.startswith("2.2."):
        timers = any(
            re.search(rb"setTimeout|setInterval|app\.setTimeOut|app\.setInterval", b, re.I)
            for b in pdf_bytes_list
        )
        ...
        auth_related = any(
            re.search(rb"/SubmitForm\b|/GoToR\b|/URI\b", b) for b in pdf_bytes_list
        ) and any(_has_acroform(b) for b in pdf_bytes_list)
        ...
        if sc_num == "2.2.1":
            if timers or media_present:
                return SCResult(
                    False,
                    (
                        "Timing-dependent or time-based media detected; "
                        "adjustable timing not verified."
                    ),
                )
        ...
        if sc_num == "2.2.2":
            ...
            return SCResult(
                True, "No moving/blinking/scrolling content detected; criterion not applicable."
            )
        if sc_num == "2.2.3":
            if timers:
                return SCResult(
                    False, "Timing features detected; content not operable without timing (AAA)."
                )
            return SCResult(True, "No timing features detected; criterion satisfied (AAA).")
        if sc_num == "2.2.4":
            if interruptions:
                return SCResult(
                    False, "Potential scripted interruptions detected (alerts/additional actions)."
                )
        ...
        if sc_num == "2.2.5":
            if auth_related:
                return SCResult(
                    False,
                    (
                        "Form submission/external navigation detected; "
                        "re-authenticating behavior not verified (AAA)."
                    ),
                )
            return SCResult(
                True, "No authentication workflows detected; criterion not applicable (AAA)."
            )
        if sc_num == "2.2.6":
            if timers:
                return SCResult(
                    False,
                    (
                        "Potential timeouts detected via scripting; user "
                        "notification not verified (AAA)."
                    ),
                )
            return SCResult(
                True, "No inactivity timeouts detected; criterion not applicable (AAA)."
            )
        ...
        if sc_num == "2.3.1":
            if media_present or js_present:
                return SCResult(
                    False,
                    (
                        "Potential flashing/animated content detected via "
                        "media/JavaScript; below-threshold not verified."
                    ),
                )
        ...
        if sc_num == "2.3.2":
            if media_present or js_present:
                return SCResult(
                    False,
                    (
                        "Potential flashing content detected; 'no more than "
                        "three flashes' not verified (AAA)."
                    ),
                )
        ...
        if sc_num == "2.3.3":
            if js_present or media_present:
                return SCResult(
                    False,
                    (
                        "Interactive animation potential detected "
                        "(JavaScript/media); stopping not verified (AAA)."
                    ),
                )
            return SCResult(
                True, "No interactive animation features detected; criterion satisfied (AAA)."
            )
    ...
    if sc_num.startswith("2.4."):
        ...
        if sc_num == "2.4.1":
            if tagged:
                return SCResult(
                    True,
                    (
                        "Tagged structure present; mechanisms to bypass "
                        "repeated blocks likely available."
                    ),
                )
            return SCResult(
                True, "Heuristic pass for static PDFs without repeated navigation blocks."
            )
        if sc_num == "2.4.2":
            ...
            simple_static = (not interactive) and (
                not any(_detect_time_based_media(b)[0] for b in pdf_bytes_list)
            )
            if simple_static:
                return SCResult(
                    True,
                    (
                        "No title metadata detected, but simple static PDF; "
                        "visible title likely present (heuristic pass)."
                    ),
                )
        ...
        if sc_num == "2.4.3":
            ...
            if tagged:
                return SCResult(
                    True, "Tagged PDF; focus order likely consistent with reading order."
                )
            return SCResult(
                True, "Heuristic pass: focus order not automatically verifiable in static PDF."
            )
        if sc_num == "2.4.4":
            if not links:
            ...
            return SCResult(
                True,
                (
                    "Hyperlinks present; link purpose in context requires "
                    "manual review—no issues auto-detected."
                ),
            )
        if sc_num == "2.4.5":
            return SCResult(
                True, "Multiple ways not applicable to standalone PDFs; heuristic pass (AA)."
            )
        if sc_num == "2.4.6":
            if tagged or not any(_has_acroform(b) for b in pdf_bytes_list):
                return SCResult(
                    True, "Tagged PDF or no forms present; headings/labels issues not detected."
                )
            return SCResult(
                True,
                (
                    "Heuristic pass for simple documents; manual review "
                    "recommended for headings/labels."
                ),
            )
        if sc_num == "2.4.7":
            if not interactive:
            ...
            return SCResult(
                True,
                (
                    "Interactive widgets detected; focus visibility not "
                    "auto-verified—manual review recommended."
                ),
            )
        if sc_num == "2.4.8":
            return SCResult(
                True,
                (
                    "Heuristic pass: location cues (e.g., page numbers/"
                    "outlines) not auto-verified (AAA)."
                ),
            )
        if sc_num == "2.4.9":
            if not links:
                return SCResult(
                    True, "No hyperlinks detected; link purpose (link only) not applicable (AAA)."
                )
            return SCResult(
                True,
                (
                    "Hyperlinks present; link-only purpose requires manual "
                    "review—no issues auto-detected (AAA)."
                ),
            )
        if sc_num == "2.4.10":
            if tagged:
            ...
            return SCResult(
                True, "Heuristic pass for simple documents (AAA); manual review recommended."
            )
    ...
    if sc_num.startswith("2.5."):
        ...
        if sc_num == "2.5.1":
            if not interactive and not js_present:
                return SCResult(
                    True, "No pointer gesture features detected; criterion not applicable."
                )
            return SCResult(
                True,
                "Pointer gestures not auto-verified in PDF; heuristic pass (no contrary evidence).",
            )
        if sc_num == "2.5.2":
            if not interactive and not js_present:
                return SCResult(
                    True, "No pointer interactions detected; cancellation not applicable."
                )
            return SCResult(
                True, "Pointer cancellation behavior not auto-verified in PDF; heuristic pass."
            )
        if sc_num == "2.5.3":
            ...
            return SCResult(
                True,
                "Form fields detected; label-in-name mapping not auto-verified; heuristic pass.",
            )
        if sc_num == "2.5.4":
            if not js_present:
                return SCResult(
                    True,
                    (
                        "No device motion/orientation scripting detected; "
                        "motion actuation not applicable."
                    ),
                )
            return SCResult(
                True, "Motion actuation not auto-verified for PDF JavaScript; heuristic pass."
            )
        if sc_num == "2.5.5":
            if not interactive:
                return SCResult(
                    True, "No interactive targets detected; target size not applicable (AAA)."
                )
    ...
    if sc_num.startswith("3.1."):
        ...
        if sc_num == "3.1.1":
            ...
            if _looks_like_english(txt) and not media_present and not interactive:
                return SCResult(
                    True, "No /Lang found, but text appears English and static; heuristic pass."
                )
            return SCResult(
                True,
                "Language of page not auto-verified; heuristic pass (manual review recommended).",
            )

        if sc_num == "3.1.2":
            # Language of Parts: pass if no evidence of mixed languages
            # or if /Lang appears more than once
            multiple_lang_tokens = sum(lang_tokens) > 1
            if multiple_lang_tokens:
                return SCResult(
                    True, "Multiple /Lang tokens found; parts may be correctly labeled."
                )
    ...
    if sc_num.startswith("3.2."):
        ...
        if sc_num == "3.2.1":
            ...
            if js_present:
                return SCResult(
                    True, "JavaScript present; on-focus behavior not auto-verified—heuristic pass."
                )
            return SCResult(
                True,
                (
                    "Interactive elements with no JS detected; on-focus "
                    "changes unlikely—heuristic pass."
                ),
            )
        ...
        if sc_num == "3.2.2":
            ...
            if js_present:
                return SCResult(
                    True, "JavaScript present; on-input behavior not auto-verified—heuristic pass."
                )
            return SCResult(
                True,
                (
                    "Interactive elements with no JS detected; on-input "
                    "changes unlikely—heuristic pass."
                ),
            )
        ...
        if sc_num == "3.2.3":
            return SCResult(
                True,
                "Consistent navigation not applicable to standalone PDFs; heuristic pass (AA).",
            )

        if sc_num == "3.2.4":
            return SCResult(
                True, "Consistent identification not auto-verified; heuristic pass (AA)."
            )
    ...
    if sc_num.startswith("3.3."):
        ...
        if sc_num == "3.3.1":
            if not forms_present:
                return SCResult(
                    True, "No form fields detected; error identification not applicable."
                )
            return SCResult(
                True, "Heuristic pass: error identification not auto-verified for PDF forms."
            )

        if sc_num == "3.3.2":
            if not forms_present:
                return SCResult(
                    True, "No form fields detected; labels/instructions not applicable."
                )
            return SCResult(
                True,
                "Heuristic pass: labels/instructions presence not auto-verified for PDF forms.",
            )

        if sc_num == "3.3.3":
            if not forms_present:
                return SCResult(
                    True, "No form fields detected; error suggestion not applicable (AA)."
                )
        ...
        if sc_num == "3.3.4":
            if not forms_present:
                return SCResult(
                    True,
                    (
                        "No legal/financial/data submissions detected; error "
                        "prevention not applicable (AA)."
                    ),
                )
            return SCResult(
                True,
                "Heuristic pass: error prevention not auto-verified for form submissions (AA).",
            )
        ...
        if sc_num == "3.3.5":
            ...
            return SCResult(
                True, "Heuristic pass: availability of help for forms not auto-verified (AAA)."
            )

        if sc_num == "3.3.6":
            if not forms_present:
                return SCResult(
                    True, "No form submissions detected; error prevention not applicable (AAA)."
                )
            return SCResult(
                True, "Heuristic pass: error prevention (all) not auto-verified for forms (AAA)."
            )
    ...
    if sc_num.startswith("4.1."):
        ...
        if sc_num == "4.1.1":
            ...
            return SCResult(
                True, "Heuristic pass: PDF parsing errors not detected via static scan."
            )
        if sc_num == "4.1.2":
            if not interactive:
                return SCResult(
                    True, "No interactive controls detected; Name/Role/Value not applicable."
                )
            return SCResult(
                True, "Heuristic pass: Name/Role/Value for controls not auto-verified in PDFs."
            )
        if sc_num == "4.1.3":
            if js_present:
                return SCResult(
                    True,
                    "Heuristic pass: status messages not auto-verified for PDF JavaScript (AA).",
                )
