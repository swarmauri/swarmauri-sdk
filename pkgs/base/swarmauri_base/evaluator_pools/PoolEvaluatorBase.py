from typing import Dict, Sequence, List, Callable, Optional, Any, TypeVar, Generic
...
from swarmauri_core.evaluator_pools.IPoolEvaluator import (
    IPoolEvaluator, 
    EvaluationResult
)
...
class PoolEvaluatorBase(IPoolEvaluator, ComponentBase, Generic[P]):
    """
    Abstract base class implementing thread-safe registry, dispatch, and result aggregation
    for evaluator pools.
    
    This class provides reusable logic for managing sets of evaluation functions, including
    parallel execution, result aggregation, and evaluator registration. It implements the
    IPoolEvaluator interface and provides default implementations for most methods.
    
    Attributes:
        _evaluators: Dictionary mapping evaluator names to evaluator instances
        _lock: Thread lock for synchronizing access to shared resources
        _executor: ThreadPoolExecutor for parallel evaluation
        _aggregation_func: Function used to aggregate scores from multiple evaluators
    """
    ...
    def __init__(self):
        ...
        super().__init__()
        self._evaluators: Dict[str, IEvaluate] = {}
        self._lock = threading.RLock()
        self._executor = None
        self._aggregation_func: Callable[[Sequence[float]], float] = lambda scores: sum(scores) / len(scores) if scores else 0.0
    ...
    def add_evaluator(self, evaluator: IEvaluate, name: Optional[str] = None) -> str:
        ...
        if not isinstance(evaluator, IEvaluate):
            logger.error(f"Attempted to add non-IEvaluate object to pool: {type(evaluator)}")
    ...
    def evaluate_all(self, programs: Sequence[P], **kwargs) -> Sequence[EvaluationResult]:
        """
        ...
        try:
            ...
            for result in final_results:
                if result.scores:
                    result.aggregate_score = self.aggregate(list(result.scores.values()))
    ...
    async def evaluate_async(self, programs: Sequence[P], **kwargs) -> Sequence[EvaluationResult]:
        """
        ...
        try:
            ...
            result = await loop.run_in_executor(
                self._executor,
                partial(self.evaluate_all, programs, **kwargs)
            )
            return result
        except Exception as e:
    ...
    def _dispatch(self, programs: Sequence[P]) -> Sequence[EvaluationResult]:
        """
        ...
        if not evaluator_items:
            ...
            return [EvaluationResult(p, {}, {}) for p in programs]
        ...
        for program in programs:
            ...
            results.append(EvaluationResult(program, scores, metadata))
    ...
    def post_process(self, results: Sequence[EvaluationResult]) -> Sequence[EvaluationResult]:
        """
