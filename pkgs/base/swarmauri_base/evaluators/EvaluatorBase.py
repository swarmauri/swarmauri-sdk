...
from swarmauri_core.evaluators.IEvaluate import IEvaluate, EvaluationError
...
class EvaluatorBase(IEvaluate, ComponentBase):
    ...
    def evaluate(self, program: Program, **kwargs) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Evaluation completed in {execution_time:.4f}s with score {score:.4f}")
    ...
    def aggregate_scores(self, scores: List[float], metadata_list: List[Dict[str, Any]]) -> Tuple[float, Dict[str, Any]]:
        ...
        if metadata_list:
            common_keys = set.intersection(*[set(meta.keys()) for meta in metadata_list])
            for key in common_keys:
                ...
                if key == "execution_time":
                    ...
                    aggregated_metadata[key] = sum(meta[key] for meta in metadata_list) / len(metadata_list)
