...
def parse_junit_xml(xml_path):
    ...
    for testcase in root.findall(".//testcase"):
        ...
        for error in testcase.findall("error"):
            ...
            error.text.lower() if error.text else ""
...
if __name__ == "__main__":
    ...
    print(
        f"Integration Failures: {results['integration_failures']}/{results['total_cases']}"
    )
    print(
        f"Acceptance Failures: {results['acceptance_failures']}/{results['total_cases']}"
    )
    print(
        f"Experimental Failures: {results['experimental_failures']}/{results['total_cases']}"
    )
    print(
        f"Collection Failures: {results['collection_failures']}/{results['total_cases']}"
    )
    ...
    try:
        print(
            f"Pass Rate: {(1 - int(failures) / int(results['total_cases'])) * 100:.2f}%"
        )
    except ZeroDivisionError:
        print("Pass Rate: 0 out of 0")
